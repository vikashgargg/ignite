...
def remote():
    ...
    original_enable_join_reorder = os.environ.get("ZELOX_OPTIMIZER__ENABLE_JOIN_REORDER")

    # `optimizer.enable_join_reorder` (experimental) is configured via env at startup.
    os.environ["ZELOX_OPTIMIZER__ENABLE_JOIN_REORDER"] = "true"
    ...
    try:
        ...
        if original_enable_join_reorder is None:
            os.environ.pop("ZELOX_OPTIMIZER__ENABLE_JOIN_REORDER", None)
        else:
            os.environ["ZELOX_OPTIMIZER__ENABLE_JOIN_REORDER"] = original_enable_join_reorder
