"""vajra-pyspark CLI — start/stop/smoke-test a Vajra server."""
...
def cmd_start(args: argparse.Namespace) -> int:
    bin_path = os.environ.get("VAJRA_BIN", "vajra")
    cmd = [bin_path, "server", "--ip", args.ip, "--port", str(args.port)]
    if args.mode:
        os.environ["SAIL_MODE"] = args.mode
    if args.workers:
        os.environ["SAIL_CLUSTER__WORKER_INITIAL_COUNT"] = str(args.workers)
    if args.auth_token:
        cmd += ["--auth-token", args.auth_token]
    print(f"Starting Vajra server: {' '.join(cmd)}")
...
def cmd_smoke(args: argparse.Namespace) -> int:
    ...
    tests = [
        ...
        ("SELECT 'Vajra' AS name", lambda df: df.collect()[0]["name"] == "Vajra"),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog="vajra-pyspark",
        description="Vajra PySpark CLI — manage a local Vajra server",
    )
    sub = parser.add_subparsers(dest="command")

    # start
    p_start = sub.add_parser("start", help="Start a Vajra server")
