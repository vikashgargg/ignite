...
```bash
cargo test -p zelox-catalog-hms --lib
```
...
- `crates/zelox-catalog-hms/tests/common/mod.rs`
- `crates/zelox-catalog-hms/tests/kerberos_integration_tests.rs`
- `crates/zelox-catalog-hms/tests/fixtures/kerberos-kdc/`
...
The GSSAPI FFI bindings in `crates/zelox-catalog-hms/src/security/gssapi_bindings.rs` were generated by `bindgen` from the system GSSAPI headers. To regenerate for a newer GSSAPI version:

```bash
bindgen /usr/include/gssapi.h -o crates/zelox-catalog-hms/src/security/gssapi_bindings.rs
```
