...
| `sail-physical-plan/src/streaming/watermark.rs` | `WatermarkExec`: emits `Watermark` markers = `max(event_time)−delay`. **Per-partition (Flink)**: `with_partition_watermark(col,N)` → MIN over partitions, withheld until all N seen (`update_per_partition`); default global. Fixes premature window close |
| **Watermark/source wiring** | `WatermarkNode` created at `resolver/query/misc.rs:205` (PRE-rewriter); `WatermarkExec` built at `planner.rs:~383` (per-partition enabled here via `VAJRA_WM_PARTITIONS` prove-it); realtime Kafka source pinned `parallelism=1` at `kafka/reader.rs:279` (single-instance EO commit) — the per-partition-watermark gap. `count_kafka_partitions` (async) |
