# Real-world head-to-head — Zelox vs Spark vs Flink (2026-06-11)

One AWS **c7g.2xlarge** Graviton node (8 vCPU, 15 GiB, ap-south-1), Ubuntu 22.04 ARM64.
Zelox = the `phase5` image (all recent streaming work — dropDuplicates, keyed windows,
...
| Workload | Zelox | Spark 3.5.3 | Flink 1.18 | Read |
|---|--:|--:|--:|---|
| **Batch** (20M ⨝ 100k + group-by, warm) | **0.28 s** | 0.70 s | — | **Zelox ~2.5×** |
| **Streaming windowed** (1s tumbling count, single-partition) | **~5.5M rows/s** | ≥3M/s (capped) | ~3.55M rows/s | **Zelox ~1.55× Flink** |
...
engines (range-generated, no I/O confound), warm. Zelox **0.28 s vs Spark 0.70 s ≈ 2.5×**,
both correct (20M rows, 10 groups). Consistent with the DataFusion-core advantage.

### Streaming windowed — Zelox ahead, with a methodology caveat
1-second tumbling windowed count. Zelox's processing **saturates ~5.5M rows/s** (window
...
The run surfaced concrete Zelox streaming gaps to close before a clean 3-engine streaming
...
MacBook Air; Zelox = persistent `zelox server` (debug build); Spark 3.5.3 `local[*]` with
`spark-sql-kafka-0-10`. Same input, same sink format — a true like-for-like.

| Mode | Zelox | Spark 3.5.3 | Read |
|---|--:|--:|---|
...
| **continuous → durable file sink, exactly-once** | **✅ 50,000 rows, exactly-once** (+ EO **across restart** measured separately: wave1 5k → stop → wave2 5k → 10,000 distinct, contiguous) | **❌ not possible** — `java.lang.IllegalStateException: Unknown type of trigger: ContinuousTrigger(1000)`; Spark Continuous sinks are **Kafka/Memory/Console only, at-least-once** (official docs) | **capability Zelox has and Spark does not** |
...
- The **headline is the capability gap**, not the micro-batch wall-clock. Spark *cannot* run
  ...
  even where it does run; Zelox does continuous **exactly-once** to parquet, recoverable across
...
- The micro-batch **0.51 s vs 8.78 s** ratio is inflated by Spark's local-mode/JVM startup; a warm
  Spark narrows it. But Zelox's **~98.7k rows/s** sustained on a debug build (no-JVM, Arrow core) is
...
- Single node, single Kafka partition. **Multi-partition / distributed** realtime is F2/F3 (the
  ...
  the next external head-to-head (the Iceberg-sink run earlier showed Zelox 0.26 s vs Flink 20.27 s
  on equivalent input).
...
container bind) — exactly the punch-list to make Zelox's streaming industry-grade. File-source
