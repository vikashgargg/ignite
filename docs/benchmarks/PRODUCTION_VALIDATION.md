...
A same-machine validation of Zelox's core claims on **real-world data** (not synthetic),
...
on the **same machine** (8-core laptop). Zelox is a **debug build** here (conservative —
...
| Query | Results identical to Spark? | Zelox (debug) | Spark 3.5.3 |
|---|:--:|--:|--:|
...
Zelox was 1.48× faster overall** (4/6 queries faster); a release build widens this
substantially (see the published TPC-H ~36× / ClickBench numbers).

## 2. Fewer resources (peak memory, same workload)
| Engine | Peak RSS | |
|---|--:|---|
| **Zelox** | **157 MB** | single static binary, no JVM |
...
## Honest scope
- **Debug build, single 8-core laptop, 3M-row dataset.** Conservative for Zelox (release
...
On real data, same machine: **Zelox returns identical results to Spark on 6/6 queries,
