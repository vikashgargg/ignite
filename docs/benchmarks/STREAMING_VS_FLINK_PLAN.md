# Zelox vs Flink (streaming) + Spark (batch) — head-to-head: measured locals + the credible EKS plan

**Date:** 2026-06-16. **Engine state:** Zelox does distributed batch + distributed *stateful* streaming
...
## 1. Measured locally (release build, 8-core Apple M-series, single node) — Zelox

| Workload | Zelox | Spark 3.5.3 (`local[*]`) | Notes |
|---|--:|--:|---|
...
| **Streaming** 5M-row keyed event-time tumbling-window COUNT over a shared CSV (availableNow) | **1.63 s (~3.07M rows/s)**, 4850 windows | — | Zelox output verified (4850 windows; semantics match Spark, see harness). Correct + measured. |

These Zelox numbers are real and reproducible. Per-core ≈ 3.07M/8 ≈ **~384k rows/s/core** for the
...
**Conclusion:** a *credible, publishable* Zelox-vs-Flink streaming head-to-head must run on **consistent
...
2. **Streaming (vs Flink):** run **Nexmark** on Flink (official `nexmark-flink`) AND the equivalent
   query logic on Zelox via the Spark API (Zelox speaks Spark SQL/DataFrame). Same 100M-event workload,
...
snapshot); the benchmark harness (Nexmark-on-Zelox query ports) is built alongside.
