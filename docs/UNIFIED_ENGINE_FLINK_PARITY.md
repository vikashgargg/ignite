...
| F3 | **Checkpointing via Chandy-Lamport aligned barriers** (global consistent snapshot) | Distributed exactly-once | 🟡 **alignment primitive built + unit-tested (2026-06-15):** `StreamBarrierAlignExec` (N→1) collects a broadcast `Checkpoint{epoch}` from all N inputs, **blocks each input's post-barrier data until aligned**, forwards a single barrier (Flink's "barriers never overtake records" rule; data preserved, verified). Per-source offset-WAL + per-operator state snapshot on EndOfData works single-node micro-batch. *Remaining:* wire alignment into a continuous multi-partition/stateful pipeline (needs streaming-repartition planning + continuous stateful ops) + a distributed barrier coordinator | **P0 (with F2)** |
