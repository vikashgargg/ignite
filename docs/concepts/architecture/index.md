...
Zelox is designed to be a high-performance, user-friendly compute engine that supports both local and cluster modes.
...
The architecture of Zelox allows for smooth transitions between these modes, enabling users to scale their workloads from laptops to clusters with minimal friction.

Zelox serves as a Spark Connect server that maintains a bidirectional gRPC connection with the PySpark client.
The PySpark client submits execution requests and receives execution results through this connection.

The Zelox server performs semantic analysis on the submitted execution requests and generates an optimized physical execution plan, which is discussed in more detail on the [Query Planning](../query-planning/) page.
...
In local mode, Zelox runs as a single process.
...
In cluster mode, Zelox forms a distributed system involving the Zelox server and multiple Zelox **workers**.
The Zelox server in cluster mode can support multiple sessions, each powered by a **cluster job runner**. Each cluster job runner owns a **driver** that schedules the distributed physical plan containing multiple **stages**. Each stage can have multiple **tasks**, each processing a partition of data. The tasks are sent to the workers for execution.

Zelox operates with a separation of concerns between the **control plane** and the **data plane**.

For the control plane, the internal Zelox gRPC protocol is used for communication between the driver and workers.
The driver and workers act as both gRPC servers and clients.
Note that the Zelox workers do not communicate with each other in the control plane.
...
1. The PySpark client submits an execution request to the Zelox server.
