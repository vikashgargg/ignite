# Tri-engine benchmark matrix — Zelox vs Spark (batch) vs Flink (streaming), every dimension

**Goal (2026-07-01):** ONE comprehensive, fair, like-for-like baseline of Zelox against the two engines
...
- **Streaming vs Flink 1.19:** throughput 5.28M vs 5.78M (1.10× slower) · memory 10.34 vs 8.58 GiB
  (1.20× MORE) · **latency competitive, Zelox TAIL p999/max BETTER** (126/129 vs 127/131ms = no-GC win).
- **Batch vs Spark 3.5.3 (TPC-DS):** Zelox **97/99 query coverage** (Q5/Q9 compat gaps) · memory **0.32 vs
...
| # | Dimension | Metric | Zelox (have) | Flink (have) | Status / need |
|---|---|---|---|---|---|
...
| S3 | **Latency** | p50/p99/p999 e2e | p50=62 p99=119 p999=126 max=129 | p50=53 p99=110 p999=127 max=131 | ✅ **MEASURED (EKS 2026-07-01, 20k/s): competitive; Zelox TAIL p999/max SLIGHTLY BETTER (no-GC)** |
...
| # | Dimension | Metric | Zelox (have) | Spark (have) | Status / need |
|---|---|---|---|---|---|
| B1 | TPC-H | query wall, SF | SF-1 1.78s (local) | 63.46s (3.5.3) | ✅ ~36× local — re-confirm @ bigger SF on EKS, fair |
| B2 | TPC-DS | coverage + wall + mem | 97/99 cov, 0.32 GiB | 2.5 GiB | ⚠️ **EKS 2026-07-01: Zelox 97/99 queries (Q5 schema, Q9 type-cmp) = strong Spark-compat; MEMORY Zelox 0.32 vs Spark 2.5 GiB = ~8× less (no-JVM). BUT tpcds_score.py `LIMIT 10000` = tiny data → times MEANINGLESS; perf-at-SF needs gen fix (remove LIMIT + real dsdgen at SF)** |
...
**Phase 1 — LOCAL engine-soundness (free, no baseline needed; is Zelox itself prod-grade?):**
...
- B1+B4 TPC-H SF-100 vs Spark: `k8s/eks/spark-bench-job.yaml` runs the SAME `tpch_distributed.py` on
  Spark 3.5.3 local[16] (after scaling Zelox→0 = same node, fair) + cgroup `memory.peak`. Zelox side =
  `tpch_distributed.py` vs Zelox server.
...
mirrors `stream_latency_query.py`); Zelox latency = `stream_latency.sh`.
**ALSO REUSABLE (confirmed):** B2 TPC-DS = `scripts/tpcds_score.py` (Zelox) + spark-bench pattern; B3
ClickBench = `scripts/clickbench.py` (Zelox, downloads hits parquet) + spark-bench pattern. So ALL batch
dims (B1/B2/B3 + memory) = existing Zelox script + the `spark-bench-job.yaml` pattern parameterized by
script/args. `lat_probe.py` (S3) BUILT+VALIDATED (Zelox local p50=43 p99=59 p999=141ms, debug).
...
- **Batch:** `k8s/eks/cluster-sf100.yaml` (+ `zelox-sf100.yaml` Zelox deploy, `tpch-gen-job.yaml` data,
...
- **Streaming phase:** `eks_stream_headtohead.sh` (S1 throughput + S2 mem vs Flink) + ADD latency S3
  (deploy `flink-sql-latency.sql` continuous + `lat_probe.py` ENGINE=flink; Zelox passthrough +
  `lat_probe.py` ENGINE=zelox). Teardown.
- **Batch phase:** `cluster-sf100` + `zelox-sf100` → `tpcds_score.py` vs Zelox (B2 power test) + scale
  Zelox→0 → `spark-bench-job.yaml` parameterized for `tpcds_score.py` (Spark baseline) + B1 TPC-H +
  mem. Teardown.
...
## Deliverable
`scripts/tri_engine_scorecard.sh` orchestrating Phase-2 on EKS (Zelox+Flink+Spark), emitting the full
