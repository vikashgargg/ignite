...
> Zelox embeds DataFusion as its execution core. DataFusion **54.0.0** is released
> (https://datafusion.apache.org/download.html, release tracking apache/datafusion#21080).
> This plans the upgrade and where 54.0.0 buys Zelox concrete wins. Current pin: **53.1.0**
> (workspace `Cargo.toml`, ~30 `datafusion*` crates incl. `datafusion-proto` used by our codec).

## What 54.0.0 brings that directly helps Zelox

| 54.0.0 change | Zelox benefit |
|---|---|
...
| **Lateral joins; lambda expressions + `array_transform`; Extension Type Registry** | Spark-compat surface area (lateral, higher-order fns) + a clean hook for **Zelox-native types** (vector/embedding for the AI-native north star) via the type registry. |
