...
**Goal:** bound Zelox's streaming RSS ≤ Flink (measured EKS: Zelox 10.34 vs Flink 8.58 GiB = 1.20× MORE
...
## Zelox design (better than both — the combination none has)
...
⇒ explicit bound + zero-copy + optimized per-operator state + spill + no JVM = the axis where Zelox can be
...
- Made in-flight buffers env-tunable (the bound levers): `ZELOX_EXCHANGE_CHANNEL_CAP` (exchange channel
  depth, default 16) + `ZELOX_SOURCE_MAX_BATCH_BYTES` (Kafka source batch cap, default 128MiB). The mpsc
...
- **Local A/B (5M-row bounded windowed-agg, file source):** exchange cap 16→2 = peak RSS **2.35→2.08 GiB
  ...
  path — validate on the EKS re-measure with `ZELOX_SOURCE_MAX_BATCH_BYTES=32MiB` + `CAP=4`.
...
The env-tunable buffers (ZELOX_EXCHANGE_CHANNEL_CAP / SOURCE_MAX_BATCH_BYTES) stay as coarse extra knobs.
...
(→ the exact 1.20× ratio). **FIX:** `ZELOX_KAFKA_PREFETCH_KBYTES` (default 1 GiB→256 MiB) + `_MSGS`
(1M→400k), env-tunable.
...
re-measure sweeping `ZELOX_KAFKA_PREFETCH_KBYTES` ∈ {1 GiB, 256 MiB, 64 MiB}** (env, no rebuild): the
RSS-vs-throughput curve confirms the driver + picks the sweet spot. Then set default + merge.

## ✅ MEASURED (EKS 100M INSTRUMENTED sweep, 2026-07-02) — Zelox BEATS Flink memory, but NOT via prefetch
...
| Zelox @1 GiB | 7.14 GiB | **0.044 GiB** | 5.668M | ✅ 9000 |
| Zelox @256 MiB | 7.32 GiB | **0.044 GiB** | 5.750M | ✅ 9000 |
| Zelox @64 MiB | 7.33 GiB | **0.044 GiB** | 5.649M | ✅ 9000 |
...
falls behind — UNEXERCISED here). (2) **Zelox genuinely beats Flink: ~7.1–7.3 vs 8.55 GiB**, consistent,
...
`ZELOX_KAFKA_PREFETCH_KBYTES` KEPT as a defensive bound + prod-grade Kafka observability; default 64 MiB
...
≤ Flink. This is the one measured axis where Zelox currently LOSES on the streaming-bounded path.
