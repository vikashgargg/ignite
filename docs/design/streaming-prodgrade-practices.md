...
## The bar, per concern (canonical source → what prod-grade means → Zelox status)

| Concern | Canonical source | Prod-grade bar | Zelox |
|---|---|---|---|
...
| **Throughput** | Arrow columnar; Leis et al. *Morsel-Driven Parallelism* (SIGMOD 2014); Arrow Flight zero-copy shuffle; DataFusion vectorized agg; Arroyo columnar-JSON (REFERENCES §6) | Vectorized batch ops, morsel/NUMA-parallel operators, zero-copy shuffle | 🟡 **~1.05× Flink** measured (EKS 100M 2026-07-07: fused-Zelox 5.24–5.40M vs Flink 5.58M ev/s). **VAJ-T7 source-fusion MEASURED = NO beat** (fused≈unfused≈5.25M) — `from_json` already runs pre-exchange, so parse-*placement* isn't the bottleneck; the Kafka **read path + Arrow decode + exchange** are. T7 stays correct+opt-in (may help RSS). **Real levers now: VAJ-BF2 Arrow Flight exchange + Kafka read/decode profile.** VAJ-T7b simd-json DONE (153ae332). |
