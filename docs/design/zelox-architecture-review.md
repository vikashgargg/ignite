# Zelox Architecture Review — subsystem gap analysis vs Spark / Flink / DataFusion / RisingWave / Arroyo / Polars
...
- **Correctness: TIE** — Zelox 10 windows/100M byte-identical to Flink, both S3/Kafka-verified (0 mismatch).
- **Throughput: ~6–7M/s vs Flink ~10M = ~1.4×** (the old "4M/2.5×" was a harness cadence artifact — proven).
- **Latency: p50 100 vs 95ms** (~tie; kind low-parallelism showed a Zelox tail win — config-dependent).
...
- **Batch: Zelox 6.2× Spark** (200M ETL) — the proven categorical win.
...
| Subsystem | Zelox today | Leaders' proven design | Gap → ideal | Prio |
|---|---|---|---|---|
...
| **Scheduler** | Zelox task scheduler (morsel-driven) | Flink pipelined-region + credit; Spark DAG stages; DataFusion `target_partitions` | overhead unmeasured — profile before touching. **RFC-scheduler** | P1 |
...
| **Planner / optimizer** | DataFusion 54 + 3 adopted rules + Zelox streaming rewriter | Spark Catalyst; Flink Calcite; DataFusion optimizer; RisingWave | adopt DF54 v0.6.5 features (PIVOT, window_time, lambda). | P2 |
...
`ZELOX_T7_FUSE`, `ZELOX_KAFKA_LEGACY_POLL`, dual-idle→E4-only, unused sweep knobs (RT_SINGLE already
