...
Zelox 2.09–2.27M vs Flink 5.77M ev/s = 2.77×) with ONE grounded design, not trial-and-error.
...
`WatermarkExec` emits at most one watermark per `ZELOX_WATERMARK_INTERVAL_MS` (default 200 ms); `max_ts`/
...
`FlightDataEncoderBuilder`, re-merges routed DATA batches to `ZELOX_SHUFFLE_BATCH_ROWS` (target; propose
...
single-node path is untouched (`ZELOX_SHUFFLE_BATCH_ROWS=0` = off).
...
- **T1 mechanism (local/free):** distributed windowed-agg with `ZELOX_WM_PROF=1`; assert per-pod
...
- **T2 kind + MinIO (free, real pods = real Flight):** deploy the dist manifest with `shufcoal` image,
  `ZELOX_SHUFFLE_BATCH_ROWS=16384`, windowed-agg → MinIO S3; assert counts exact + batches drop +
...
flink-s3-fs-hadoop + flink-sql-parquet + flink-shaded-hadoop-2-uber + checkpoints-after-tasks-finish; Zelox
parquet). Bounded catch-up = throughput.
| Metric | Flink 1.19 | Zelox |
|---|---|---|
| Throughput | **5.07M ev/s** (19.7s) | 2.32M ev/s (43s) — Flink **2.2×** |
| Peak memory | 9.27 GiB (1 TM) | **3.70 GiB/pod** (Zelox lower) |
| Correctness | 48 files | 100M/10 windows/1000 keys exact |

**ROOT CAUSE OF THE 2.2× (Zelox per-pod WM_PROF, cpu-ms):** source_read=40–49s (≈ the 43s WALL) >> from_json
=11–12s > exchange_cpu=**0**. shuffle_recv=608s is BLOCKED-WAIT (window starves behind the source), NOT CPU.
**Zelox is SOURCE-READ BOUND** — Kafka consume + Arrow batch build dominates; the window is idle waiting.
...
The frame-pointer pprof profiler (ZELOX_PPROF_SECS; commit ee382309) run on a single-process local-cluster
...
local-cluster --workers 4, ZELOX_DISTRIBUTED_STREAM=1, 5M Kafka events → windowed-agg → parquet on MinIO S3 →
...
+ throughput both green on real object storage. NOTE: the zelox DOCKER image streaming-Kafka read is flaky on
...
mini-batch) so NO competitive claim from it. Zelox ~4.4M on S3 at 100M. Cluster torn to $0. Fix kept (correct
...
sequencing I got wrong the first time: I'd left Zelox holding the node so flink-tm was Pending → job ran
...
| Zelox certprod+fix, WARM (/data local, tiny output) | **6.74 / 6.67M ev/s** | ~15s | steady-state |
| Zelox COLD (1st run) | 3.05M | 32.8s | JIT/connection warmup |
**Flink ~1.4x faster than warm Zelox.** Output is tiny both (~9000 rows = windows×keys), so blackhole vs
/data-parquet is negligible = fair. Notes: (a) BOTH engines beat their own published numbers (Zelox 6.7M >
pub 4.92M; Flink 9.37M > pub 5.67M) — the published "1.15x behind" understated Flink (likely a sequencing/
measurement artifact like mine). (b) Zelox COLD-START is 2x slower than warm — a real prod axis (first-batch
latency). (c) counts 9 windows/90M (ZELOX_COMPLETE_ON_END not set here = the known last-window completeness
