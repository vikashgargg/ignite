...
- `flink-jm` stuck **Pending** because `zelox-client` squatted 1 CPU on the 8-vCPU kafka node (scheduling).
...
1. **Final-window completeness** — Zelox emits 9 windows / 90M where Flink emits 10 / 100M: Zelox does not
...
2. **Realtime Kafka→Kafka passthrough throughput** — Zelox ~1.3K/s p50=257ms vs Flink 20K/s p50=98ms (EKS,
   ...
   **~122K/s** (single ThreadedProducer, librdkafka-batched). Windowed-agg reads Kafka at **5.5M/s**. So the
   ...
   data); (b) per-stage profile the continuous path (ZELOX_WM_PROF); (c) design from Flink pipelined-exchange
