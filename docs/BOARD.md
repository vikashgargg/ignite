...
| **Streaming throughput** | — | `<` | 🟡 | **ROOT CAUSE ISOLATED + work-line MERGED to main 2026-07-16 (8e294cd8).** Distributed gap = **Kafka source CONSUME rate**: Vajra `StreamConsumer` ~4M/s vs Flink `KafkaSource` ~10M/s (~2.5×); transport/shuffle/serde/JVM all RULED OUT (G1 prefetch +4% marginal, G2 raw-TCP reinvents HTTP/2 — both removed). **FLIP-27 batched-drain landed (+23%, 3.24→3.98M/s), counts EXACT.** kind T2 re-verified post-merge: batch 3M EXACT + realtime mid-stream emission Flink-parity + 15000-row EXACT, 0 OOM. NEXT beat-lever = `BaseConsumer` dedicated-fetch-thread (FLIP-27) to lift 4M→~10M | **VAJ-BF2** |
