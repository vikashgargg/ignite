...
7. For the SQL `TIME` type, the optional <code><SyntaxText raw="<p>" /></code> parameter specifies the precision of the time value. Spark 4.0 supports precision `0`, `3`, and `6` (second, millisecond, and microsecond). The default value is `6` (microsecond). The `TIME` type is available in Spark 4.0 and later.
