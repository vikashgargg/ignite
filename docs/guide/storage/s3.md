...
Zelox supports reading and writing data to AWS S3 and S3-compatible object storage services (such as [MinIO](https://min.io/) or Cloudflare R2) using the `s3://`, `s3a://`, or `https://` URI schemes.

For example, `s3://bucket/path/to/data` refers to the `path/to/data` path in the `bucket` bucket.
Zelox determines whether the path refers to a single object or a key prefix.
If the path turns out to be a key prefix, we assume the key prefix is followed by `/` and represents a directory.

## URI Formats

Zelox supports the following URL formats for S3 and S3-compatible object storage services:
...
1. The `s3a` URI scheme has no difference from the `s3` URI scheme in Zelox. The `s3a` scheme is provided for compatibility with Spark applications that use the Hadoop S3A connector.
...
All AWS credential providers work out-of-box in Zelox.
...
::: info
`AWS_SKIP_SIGNATURE` is a Zelox-specific environment variable, not part of standard AWS SDKs.
:::
...
export AWS_SECRET_ACCESS_KEY="zeloxing"
