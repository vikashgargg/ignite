...
The relative file paths are relative to the current working directory of the Zelox server.
...
::: info
Zelox is agnostic whether the data is stored on a local disk or a network drive, as long as the data appears in a file system mounted on the host.
:::
