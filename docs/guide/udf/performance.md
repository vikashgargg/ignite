...
In general, the Zelox implementation of PySpark UDFs and UDTFs is more efficient than the original implementation. The Python interpreter running the UDF is in the same process as the Rust-based execution engine. Therefore, there is no data (de)serialization overhead, while the JVM-based implementation has to move data between the JVM and the Python worker.

For scalar UDFs, the user-provided Python function is called on each input row. This is usually less efficient than built-in functions which can potentially be applied in a vectorized manner in Zelox, where the Arrow columnar in-memory format is used.
...
In Zelox, the internal in-memory data in Arrow format is converted to Pandas objects before Python function invocation, and the result is converted back to the Arrow format. Such conversion can be zero-copy, but data copy occurs when the `pyarrow` library decides it is unavoidable.
...
In Zelox, the implementation does not involve any data copy. The Python function and the Rust-based execution engine share Arrow data directly via pointers.
...
To ensure full parity with PySpark, such wrappers are also used in Zelox.
