...
You can run PySpark scripts via the `zelox spark run` command.

Zelox is an open-source unified and distributed multimodal computation framework
that can be used as a drop-in replacement for Apache Spark.
By using Zelox to run PySpark scripts, you can perform data processing tasks
with the familiar PySpark DataFrame API or Spark SQL while benefiting from
the high performance and low memory overhead of Zelox.

The script can refer to the Spark session via the `spark` variable, which
connects to a local Zelox server via the Spark Connect protocol.
The Zelox server starts instantly when you run the `zelox spark run` command,
...
```bash
echo 'spark.sql("SELECT 1 + 1").show()' | zelox spark run 2>/dev/null
```

Alternatively, you can use a heredoc for more complex PySpark scripts:

```bash
cat <<EOF | zelox spark run 2>/dev/null
...
zelox spark run -f /tmp/script.py 2>/dev/null
```
