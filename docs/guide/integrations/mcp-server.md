...
You can launch an MCP (Model Context Protocol) server using the Zelox CLI.
...
Replace `/path/to/zelox` with the absolute path to the Zelox CLI on your system.
If you have installed Zelox via `pip` in a Python virtual environment, the path is `$VENV/bin/zelox` where `$VENV` is the absolute path to the virtual environment.
Please note that you must install the PyZelox library with MCP dependencies.

```bash-vue
pip install "pyzelox[mcp]=={{ libVersion }}" "pyspark-client==4.2.0"
```

::: info
You can run `zelox spark mcp-server --help` to see the available options for launching the MCP server.
:::

Restart Claude for Desktop.
A Zelox MCP server is now managed by Claude for Desktop behind the scenes.
You will see a hammer icon in the bottom right corner of the input box.
Click on the hammer icon, and you will see a list of installed tools from the Zelox MCP server.
...
To remove the Zelox MCP server from Claude for Desktop, remove the content from the configuration file and restart Claude for Desktop.
