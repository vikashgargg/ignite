...
| File / data source (parquet/csv listing) | 🟡 | **Durable for `availableNow`/`once` (2026-06-10):** `writeStream.format("parquet")` now persists a stream to files. `create_writer` decodes the flow-event input (`FlowEventToDataExec`: skip markers, strip flow-event fields) and feeds the normal file writer, wrapped in `EmptySinkAdapterExec`. Verified: rate→parquet availableNow writes + reads back correctly. **Remaining:** continuous (unbounded) per-micro-batch file commit + `_spark_metadata` manifest for idempotent exactly-once (today the bounded writer finalizes once on stream end). |
