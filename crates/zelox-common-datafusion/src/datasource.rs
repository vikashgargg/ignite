...
pub const MERGE_FILE_COLUMN: &str = "__zelox_file_path";

/// File-local row index metadata column for row-level modifications that write deletion vectors.
pub const MERGE_ROW_INDEX_COLUMN: &str = "__zelox_file_row_index";
...
pub const OPERATION_COLUMN: &str = "__zelox_operation_type";

/// Internal column carrying pre-aggregated MERGE source row counts on
/// [`RowLevelOperationType::SourceMetric`] rows.
pub const MERGE_SOURCE_METRIC_COLUMN: &str = "__zelox_merge_source_metric";
...
/// `zelox_data_source::streaming_sink_log`). Stripped before options reach the format writer.
pub const STREAM_CHECKPOINT_OPTION: &str = "__zelox_stream_checkpoint";
...
pub const STREAM_REALTIME_INTERVAL_OPTION: &str = "__zelox_stream_realtime_interval";
