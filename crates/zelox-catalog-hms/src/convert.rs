...
use zelox_catalog::error::{CatalogError, CatalogResult};
use zelox_catalog::hive_format::{HiveDetectedFormat, HiveStorageFormat};
...
mod tests {
    ...
    use pilota::{AHashMap, FastStr};
    use zelox_catalog::hive_format::HiveStorageFormat;
    use zelox_catalog::provider::{
    ...
    fn test_validate_namespace_rejects_nested_namespaces() {
        let namespace = zelox_catalog::provider::Namespace::try_from(vec!["a", "b"]).unwrap();
    ...
    fn test_build_generic_table_rejects_missing_partition_columns() {
        ...
        assert!(matches!(
            error,
            zelox_catalog::error::CatalogError::InvalidArgument(_)
        ));
    ...
    fn test_table_to_status_reports_textfile_format() {
        let namespace = zelox_catalog::provider::Namespace::try_from(vec!["default"]).unwrap();
        let table = build_generic_table(
            "default",
            "items",
            vec![CreateTableColumnOptions {
        ...
        match status.kind {
            zelox_common_datafusion::catalog::TableKind::Table { format, .. } => {
    ...
    fn test_table_to_status_converts_partition_columns_to_identity_fields() {
        ...
        let status = super::table_to_status("hms", &namespace, &table).unwrap();
        match status.kind {
            zelox_common_datafusion::catalog::TableKind::Table { partition_by, .. } => {
                assert_eq!(
                    partition_by,
                    vec![zelox_common_datafusion::catalog::CatalogPartitionField {
    ...
    fn test_table_to_status_uses_spark_schema_and_hides_internal_properties() {
        ...
        let status = super::table_to_status("hms", &namespace, &table).unwrap();
        match status.kind {
            zelox_common_datafusion::catalog::TableKind::Table {
                format,
                columns,
                properties,
                ..
    ...
    fn test_inject_spark_metadata_with_empty_columns() {
        ...
        let status = super::table_to_status("hms", &namespace, &table).unwrap();
        match status.kind {
            zelox_common_datafusion::catalog::TableKind::Table { columns, .. } => {
                assert!(
                    columns.is_empty(),
