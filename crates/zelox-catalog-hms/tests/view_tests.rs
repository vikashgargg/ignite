...
use zelox_catalog::error::{CatalogError, CatalogObject};
use zelox_catalog::provider::{CatalogProvider, CreateViewColumnOptions};
...
async fn test_create_get_drop_view() {
    ...
    let created = catalog
        .create_view(
            namespace,
            "v_items",
            zelox_catalog::provider::CreateViewOptions {
    ...
    catalog
        .drop_view(
            namespace,
            "v_items",
            zelox_catalog::provider::DropViewOptions { if_exists: false },
        )
        .await
        .unwrap();
}
