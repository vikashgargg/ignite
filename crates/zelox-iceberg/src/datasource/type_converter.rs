...
use zelox_common::spec::{
    EXTENSION_TYPE_NAME_KEY, ZELOX_LIST_FIELD_NAME, ZELOX_MAP_FIELD_NAME, VARIANT_EXTENSION_NAME,
};
use serde_json;
        ...
        Type::List(list_type) => {
            ...
            let element_field =
                if element_field.name().trim().to_lowercase() == ICEBERG_LIST_FIELD_NAME {
                    element_field.with_name(ZELOX_LIST_FIELD_NAME.to_string())
                } else {
                    element_field
                };
...
pub fn arrow_type_to_iceberg(arrow_type: &ArrowDataType) -> Result<Type> {
    match arrow_type {
        ...
        | ArrowDataType::LargeListView(field) => {
            ...
            if element_field.name.trim().to_lowercase() == ZELOX_LIST_FIELD_NAME {
...
mod tests {
    ...
    use zelox_common::spec::{ZELOX_MAP_KEY_FIELD_NAME, ZELOX_MAP_VALUE_FIELD_NAME};
    ...
    fn test_arrow_list_to_iceberg_conversion() {
        let element_field = ArrowField::new(ZELOX_LIST_FIELD_NAME, ArrowDataType::Int64, true)
    ...
    fn test_iceberg_list_to_arrow_conversion() {
        ...
        match arrow_type {
            ArrowDataType::List(field) => {
                assert_eq!(field.name(), ZELOX_LIST_FIELD_NAME);
    ...
    fn test_arrow_map_to_iceberg_conversion() {
        let key_field = ArrowField::new(ZELOX_MAP_KEY_FIELD_NAME, ArrowDataType::Utf8, false)
            .with_metadata(HashMap::from([
                (PARQUET_FIELD_ID_META_KEY.to_string(), "1".to_string()),
                (
                    ICEBERG_ARROW_FIELD_DOC_KEY.to_string(),
        ...
        let value_field =
            ArrowField::new(ZELOX_MAP_VALUE_FIELD_NAME, ArrowDataType::Int64, true).with_metadata(
                HashMap::from([(PARQUET_FIELD_ID_META_KEY.to_string(), "2".to_string())]),
            );
        let entries_struct = ArrowDataType::Struct(vec![key_field, value_field].into());
        let entries_field = ArrowField::new(ZELOX_MAP_FIELD_NAME, entries_struct, false);
