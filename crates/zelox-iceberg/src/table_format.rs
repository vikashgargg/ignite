...
use zelox_common_datafusion::datasource::{
    find_path_in_options, OptionLayer, PhysicalSinkMode, SinkInfo, SourceInfo, TableFormat,
    TableFormatRegistry,
};
use zelox_data_source::options::gen::{IcebergReadOptions, IcebergWriteOptions};
use zelox_data_source::options::ResolveOptions;
use url::Url;
...
impl TableFormat for IcebergTableFormat {
    ...
    async fn create_writer(
        ...
        let checkpoint = zelox_common_datafusion::datasource::take_option(
            &mut options,
            zelox_common_datafusion::datasource::STREAM_CHECKPOINT_OPTION,
        );
        let streaming = zelox_common_datafusion::streaming::event::schema::is_flow_event_schema(
            &input.schema(),
        );
        ...
        let (input, mode, streaming_commit) = if let (true, Some(cp)) = (streaming, &checkpoint) {
            let batch_id = zelox_data_source::formats::file_stream::current_batch_id(cp).await;
            let decoded: Arc<dyn ExecutionPlan> =
                Arc::new(zelox_data_source::streaming_decode::FlowEventToDataExec::try_new(input)?);
        ...
        if is_streaming_write {
            ...
            Ok(Arc::new(
                zelox_data_source::streaming_decode::EmptySinkAdapterExec::new(exec),
            ))
        } else {
            Ok(exec)
        }
    ...
    // inputs should consume Zelox row intent tags to decide which rows rewrite
