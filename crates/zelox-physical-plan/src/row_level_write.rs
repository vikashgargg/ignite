...
use zelox_logical_plan::merge::RowLevelWriteNode;
...
fn build_merge_operation_override(node: &RowLevelWriteNode) -> Option<OperationOverride> {
    ...
    let matched_predicates = opts
        ...
        .map(|c| {
            let action_type = match &c.action {
                zelox_logical_plan::merge::MergeMatchedAction::Delete => "delete",
                zelox_logical_plan::merge::MergeMatchedAction::UpdateAll
                | zelox_logical_plan::merge::MergeMatchedAction::UpdateSet(_) => "update",
            }
            .to_string();
    ...
    let not_matched_by_source_predicates = opts
        ...
        .map(|c| {
            let action_type = match &c.action {
                zelox_logical_plan::merge::MergeNotMatchedBySourceAction::Delete => "delete",
                zelox_logical_plan::merge::MergeNotMatchedBySourceAction::UpdateSet(_) => "update",
            }
            .to_string();
            let predicate = c.condition.as_ref().and_then(|x| x.source.clone());
            MergePredicateInfo {
