...
use zelox_common_datafusion::streaming::event::encoding::{
...
// ---------------------------------------------------------------------------
// Throughput attribution (Phase A, env ZELOX_WM_PROF) — see docs/design/eks-throughput-capstone.md.
...
mod prof {
    ...
    pub fn enabled() -> bool {
        std::env::var("ZELOX_WM_PROF").is_ok()
    }
    pub fn mark_start() {
    ...
    pub fn dump(tag: &str) {
        ...
        let enc = zelox_common_datafusion::streaming::event::encoding::ENCODE_NS
            .load(Ordering::Relaxed);
        let fj = zelox_common_datafusion::streaming::event::encoding::FROM_JSON_NS
            .load(Ordering::Relaxed);
        let rd = zelox_common_datafusion::streaming::event::encoding::SOURCE_READ_NS
            .load(Ordering::Relaxed);
        let ex = zelox_common_datafusion::streaming::event::encoding::EXCHANGE_NS
            .load(Ordering::Relaxed);
        let exw = zelox_common_datafusion::streaming::event::encoding::EXCHANGE_WAIT_NS
            .load(Ordering::Relaxed);
        ...
        let peak_mib = zelox_common_datafusion::streaming::event::encoding::EXCHANGE_PEAK_BYTES
            .load(Ordering::Relaxed)
            / 1048576;
        let reader_peak_mib = zelox_common_datafusion::streaming::event::encoding::READER_PEAK_BYTES
            .load(Ordering::Relaxed)
            / 1048576;
        ...
        log::info!(
            "WM_PROF[{tag}] wall={}ms input_wait={}ms({}%) | STAGES(summed-cpu-ms): source_read={} from_json={} exchange_cpu={} exchange_wait={} encode={} finalize={} | rows={} | reader_peak_MiB={} exchange_peak_inflight_MiB={} => {} \
             (rank the largest STAGE = the throughput bottleneck to fix)",
            wall / 1_000_000, iw / 1_000_000, pct(iw),
            rd / 1_000_000, fj / 1_000_000, ex / 1_000_000, exw / 1_000_000, enc / 1_000_000, fz / 1_000_000, rows, reader_peak_mib, peak_mib,
...
struct AccumState {
    ...
    /// inc-ckpt.2 (gated `ZELOX_INC_CKPT`, default OFF): incremental checkpointing — the
...
impl ExecutionPlan for WindowAccumExec {
    ...
    fn execute(
        ...
        // accumulation RAM). Default 128 MiB; override via ZELOX_STREAMING_STATE_BUDGET_BYTES.
        let state_budget_bytes: usize = std::env::var("ZELOX_STREAMING_STATE_BUDGET_BYTES")
            .ok()
        ...
        // Enable only when validated for a workload: ZELOX_F5_COMPACT=1.
        let compact_enabled = std::env::var("ZELOX_F5_COMPACT").is_ok();
        ...
        let inc_ckpt_enabled = std::env::var("ZELOX_INC_CKPT").is_ok();
        // Rescale-from-checkpoint (gated ZELOX_RESCALE; uses inc-ckpt staging). On a restart that
        ...
        // restore is unchanged (no regression). Requires ZELOX_INC_CKPT staging format.
        let rescale_enabled = std::env::var("ZELOX_RESCALE").is_ok();
        // Bounded-COMPLETE flush (gated ZELOX_COMPLETE_ON_END, default OFF = Spark availableNow parity):
        ...
        let complete_on_end = std::env::var("ZELOX_COMPLETE_ON_END").is_ok();
        if prof::enabled() {
            prof::mark_start();
        }
...
mod update_mode_e2e_tests {
    ...
    // inc-ckpt.2: a Checkpoint{epoch} under ZELOX_INC_CKPT stages an INCREMENTAL snapshot (manifest
    ...
    async fn inc_ckpt_epoch_snapshot_restores_full_state() {
        std::env::set_var("ZELOX_INC_CKPT", "1");
        std::env::set_var("ZELOX_STREAMING_STATE_BUDGET_BYTES", "4096"); // tiny → force spill
        ...
        std::env::remove_var("ZELOX_INC_CKPT");
        std::env::remove_var("ZELOX_STREAMING_STATE_BUDGET_BYTES");
        assert_eq!(
            rows, n,
