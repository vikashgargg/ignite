...
fn join_state_budget() -> usize {
    std::env::var("ZELOX_STREAMING_STATE_BUDGET_BYTES")
        .ok()
    ...
    fn execute(
        ...
        let event_stream = stream::unfold(init, move |(mut merged, mut acc, mut buf, ctx)| {
            ...
            async move {
                ...
                if !acc.restored {
                    if let Some(ck) = &ck {
                        // inc-ckpt.2b (gated ZELOX_INC_CKPT): restore the incremental snapshot (residual
                        ...
                        let inc = std::env::var("ZELOX_INC_CKPT").is_ok();
                        let (lb, meta) = if inc {
                    ...
                    match merged.next().await {
                        ...
                        Some((_, Ok(FlowEvent::Marker(FlowMarker::EndOfData)))) => {
                            ...
                            if let Some(ck) = &ck {
                                ...
                                if std::env::var("ZELOX_INC_CKPT").is_ok() {
