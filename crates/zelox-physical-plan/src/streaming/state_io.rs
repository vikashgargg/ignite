...
/// REFERENCES §3/§5) — Zelox spills in Arrow with no JVM, beating RocksDB's local-disk-only model.
...
pub fn zelox_key_groups(arrays: &[ArrayRef], g: u16, n_rows: usize) -> Result<Vec<u16>> {
...
/// Filter a state batch to rows whose key-group (RECOMPUTED from the `key_cols` via `zelox_key_groups`,
...
pub fn filter_to_key_group_range_recompute(
    ...
    let kgs = zelox_key_groups(&arrays, g, batch.num_rows()).ok()?;
...
mod tests {
    ...
    // zelox_key_groups(row). Guards against hash/seed drift — the basis for rescale state ownership.
    #[test]
    fn zelox_key_groups_matches_exchange_routing() {
        use datafusion::arrow::datatypes::{DataType, Field, Schema};
        ...
        let batch =
            RecordBatch::try_new(schema.clone(), vec![Arc::new(Int64Array::from(keys.clone()))])
                .unwrap();
        let mine = zelox_key_groups(&[batch.column(0).clone()], g, batch.num_rows()).unwrap();
        ...
        bp.partition(batch.clone(), |idx, sub| {
            ...
            for r in 0..sub.num_rows() {
                ...
                assert_eq!(
                    ...
                    "zelox_key_groups must equal the exchange's BatchPartitioner partition"
                );
            }
            Ok(())
        })
    ...
    async fn rescale_recompute_redistributes_exactly() {
        ...
        let kg_of = |k: i64| -> u16 {
            ...
            zelox_key_groups(&[a], g, 1).unwrap()[0]
        };
