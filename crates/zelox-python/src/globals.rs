...
use zelox_common::config::{AppConfig, ZELOX_ENV_VAR_PREFIX};
use zelox_common::runtime::RuntimeManager;
use zelox_telemetry::telemetry::{init_telemetry, ResourceOptions};
...
impl EnvironmentSnapshot {
    ...
    const ZELOX_STATIC_ENV_VAR_PREFIXES: &[&str] = &[
        "ZELOX_RUNTIME__ENABLE_SECONDARY",
        "ZELOX_RUNTIME__STACK_SIZE",
        "ZELOX_TELEMETRY__",
    ];
    ...
    fn capture() -> HashMap<String, String> {
        env::vars()
            .filter(|(k, _)| k.starts_with(ZELOX_ENV_VAR_PREFIX))
            .collect()
    }
    ...
    pub fn warn_if_changed(&self, py: Python<'_>) -> PyResult<()> {
        ...
        changed.retain(|key| {
            Self::ZELOX_STATIC_ENV_VAR_PREFIXES
                .iter()
...
fn _shutdown_telemetry() {
    zelox_telemetry::telemetry::shutdown_telemetry();
}
