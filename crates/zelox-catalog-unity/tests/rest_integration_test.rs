...
use zelox_catalog_unity::unity::{types, Client};
use zelox_catalog_unity::UnityCatalogProvider;
use zelox_common::runtime::RuntimeHandle;
use zelox_common::spec::{
    ZELOX_LIST_FIELD_NAME, ZELOX_MAP_FIELD_NAME, ZELOX_MAP_KEY_FIELD_NAME, ZELOX_MAP_VALUE_FIELD_NAME,
};
use zelox_common_datafusion::catalog::{DatabaseStatus, TableKind};
use testcontainers::core::{ContainerPort, ContainerRequest, Mount, WaitFor};
use testcontainers::runners::AsyncRunner;
use testcontainers::{ContainerAsync, GenericImage, ImageExt};

const DEFAULT_CATALOG: &str = "zelox_test_catalog";
...
async fn test_get_non_exist_schema() {
    ...
    assert!(matches!(
        result.unwrap_err(),
        zelox_catalog::error::CatalogError::NotFound(_, _)
    ));
}
...
async fn test_create_table() {
        ...
        CreateTableColumnOptions {
            ...
            data_type: DataType::Map(
                Arc::new(Field::new(
                    ...
                    DataType::Struct(Fields::from(vec![
                        Field::new(ZELOX_MAP_KEY_FIELD_NAME, DataType::Utf8, false),
                        Field::new(ZELOX_MAP_VALUE_FIELD_NAME, DataType::Int32, true),
                    ])),
                    false,
                )),
                false,
    ...
    assert!(
        columns.contains(&zelox_common_datafusion::catalog::TableColumnStatus {
            name: "foo".to_string(),
            data_type: DataType::Utf8,
            nullable: true,
            comment: None,
