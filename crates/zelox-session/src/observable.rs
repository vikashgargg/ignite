...
use zelox_common_datafusion::system::observable::{
    Observer, SessionManagerObserver, StateObservable,
};
use zelox_server::actor::ActorHandle;
...
mod tests {
    ...
    use zelox_common_datafusion::system::catalog::SystemCatalog;
    use zelox_plan::config::PlanConfig;
    use zelox_plan::resolver::PlanResolver;
    use zelox_sql_analyzer::data_type::from_ast_data_type;
    use zelox_sql_analyzer::parser::parse_data_type;

    // This test is defined in this crate so that `zelox-catalog-system` or `zelox-common-datafusion`
    // does not need to depend on `zelox-plan` or `zelox-sql-analyzer`.
    #[test]
