...
use zelox_catalog::error::CatalogResult;
use zelox_catalog::manager::{CatalogManager, CatalogManagerOptions};
use zelox_catalog::provider::{
    CachingCatalogProvider, CatalogCacheManager, CatalogProvider, RuntimeAwareCatalogProvider,
};
use zelox_catalog_glue::{GlueCatalogConfig, GlueCatalogProvider};
use zelox_catalog_hms::{HmsCatalogConfig, HmsCatalogProvider};
use zelox_catalog_iceberg::IcebergRestCatalogProvider;
use zelox_catalog_memory::MemoryCatalogProvider;
use zelox_catalog_onelake::OneLakeCatalogProvider;
use zelox_catalog_system::{SystemCatalogProvider, SYSTEM_CATALOG_NAME};
use zelox_catalog_unity::UnityCatalogProvider;
use zelox_common::config::{AppConfig, CacheType, CatalogCacheConfig, CatalogType};
...
fn wrap_catalog_provider(
    ...
    let global_bundle = if has_global {
        ...
        if bundle.is_none() {
            let new_bundle = Arc::new(zelox_catalog::provider::CatalogCacheBundle::new(
                cache_config,
            ));
...
mod tests {
    use zelox_common::config::{AppConfig, CatalogCacheConfig, CatalogType};
