...
use zelox_catalog::manager::CatalogManager;
use zelox_catalog_system::planner::SystemTablePhysicalPlanner;
use zelox_common_datafusion::catalog::TableKind;
use zelox_common_datafusion::datasource::{SourceInfo, TableFormatRegistry};
use zelox_common_datafusion::extension::SessionExtensionAccessor;
use zelox_common_datafusion::logical_rewriter::LogicalRewriter;
use zelox_common_datafusion::rename::physical_plan::rename_projected_physical_plan;
use zelox_common_datafusion::streaming::event::schema::{
    to_flow_event_field_names, to_flow_event_projection,
};
use zelox_logical_plan::barrier::BarrierNode;
use zelox_logical_plan::file_delete::FileDeleteNode;
use zelox_logical_plan::file_write::FileWriteNode;
use zelox_logical_plan::map_partitions::MapPartitionsNode;
use zelox_logical_plan::merge::MergeIntoNode;
use zelox_logical_plan::monotonic_id::MonotonicIdNode;
use zelox_logical_plan::range::RangeNode;
use zelox_logical_plan::repartition::ExplicitRepartitionNode;
use zelox_logical_plan::schema_pivot::SchemaPivotNode;
use zelox_logical_plan::show_string::ShowStringNode;
use zelox_logical_plan::sort::SortWithinPartitionsNode;
use zelox_logical_plan::spark_partition_id::SparkPartitionIdNode;
use zelox_logical_plan::streaming::collector::StreamCollectorNode;
use zelox_logical_plan::streaming::dedup::StreamDeduplicateNode;
use zelox_logical_plan::streaming::filter::StreamFilterNode;
use zelox_logical_plan::streaming::foreach_batch::ForeachBatchSinkNode;
use zelox_logical_plan::streaming::limit::StreamLimitNode;
use zelox_logical_plan::streaming::memory_sink::MemorySinkNode;
use zelox_logical_plan::streaming::source_adapter::StreamSourceAdapterNode;
use zelox_logical_plan::streaming::source_wrapper::StreamSourceWrapperNode;
use zelox_logical_plan::streaming::stream_join::StreamJoinNode;
use zelox_logical_plan::streaming::watermark::WatermarkNode;
use zelox_logical_plan::streaming::window_accum::WindowAccumNode;
use zelox_physical_plan::barrier::BarrierExec;
use zelox_physical_plan::catalog_command::CatalogCommandExec;
use zelox_physical_plan::file_delete::create_file_delete_physical_plan;
use zelox_physical_plan::file_write::create_file_write_physical_plan;
...
use zelox_physical_plan::streaming::exchange::StreamExchangeExec;
...
use zelox_plan::catalog::CatalogCommandNode;
use zelox_plan_lakehouse::new_lakehouse_extension_planners;
...
Ensure expand_row_level_op is enabled; MERGE is currently only supported for lakehouse tables."
            ...
            if is_flow_event_schema(&input.schema()) {
                match partitioning {
                    datafusion::physical_expr::Partitioning::Hash(keys, n) if n > 1 => Arc::new(
                        zelox_physical_plan::streaming::exchange::StreamExchangeExec::try_new(
                            input.clone(),
                            keys,
                            n,
                        )?,
            ...
            let data_schema = Arc::new(
                zelox_common_datafusion::streaming::event::schema::try_from_flow_event_schema(
                    &input.schema(),
                )
                .map_err(|e| {
            ...
            );
            {
                ...
                // step: N via `ZELOX_WM_PARTITIONS`; the general path threads N from the source.
                let n_parts = std::env::var("ZELOX_WM_PARTITIONS")
                    .ok()
