...
use educe::Educe;
use log::trace;
use zelox_common_datafusion::logical_expr::ExprWithSource;
...
pub const SOURCE_PRESENT_COLUMN: &str = "__zelox_merge_source_row_present";
pub const TARGET_PRESENT_COLUMN: &str = "__zelox_merge_target_row_present";
pub const TARGET_ROW_ID_COLUMN: &str = "__zelox_merge_target_row_id";

use zelox_common_datafusion::datasource::{OptionLayer, RowLevelOperationType};
pub use zelox_common_datafusion::datasource::{MERGE_SOURCE_METRIC_COLUMN, OPERATION_COLUMN};
...
use zelox_common_datafusion::datasource::RowLevelCommand;
...
pub fn expand_merge(
    ...
    for (idx, desired) in desired_source_names.iter().enumerate() {
        let prefixed = format!("__zelox_src_{desired}");
...
fn build_insert_only_projection(
    ...
    // Source columns are prefixed with `__zelox_src_`, so target field "id" maps
    // to source column "__zelox_src_id". Keys are lowercased for case-insensitive
    // SQL resolution.
        ...
        for clause in &options.not_matched_by_target_clauses {
            ...
            let value = match &clause.action {
                MergeNotMatchedByTargetAction::InsertAll => source_exprs_by_name
                    .get(&format!("__zelox_src_{}", name.to_ascii_lowercase()))
                    .cloned()
...
fn build_merge_projection(
    ...
    // Source columns are prefixed with `__zelox_src_`, so target field "id"
    // maps to source column "__zelox_src_id". Keys are lowercased for
    // case-insensitive SQL resolution.
    let source_expr_for_target = |target_name: &str| -> Expr {
        let prefixed = format!("__zelox_src_{}", target_name.to_ascii_lowercase());
