...
use zelox_catalog::utils::{get_property, quote_name_if_needed, quote_namespace_if_needed};
...
use zelox_iceberg::utils::partition_transform::catalog_partition_field_from_iceberg;
use zelox_iceberg::{arrow_type_to_iceberg, iceberg_type_to_arrow, NestedField, StructType};
...
impl CatalogProvider for IcebergRestCatalogProvider {
    ...
    async fn create_table(
        ...
        let (name_to_id, _id_to_name) =
            zelox_iceberg::spec::SchemaBuilder::build_name_indexes(&struct_type);
        ...
        let schema = zelox_iceberg::spec::Schema::builder()
            .with_fields(fields)
    ...
    async fn create_view(
        ...
        let schema = zelox_iceberg::spec::Schema::builder()
            ...
            .build()
            .map_err(|e| CatalogError::External(format!("Failed to build schema: {e}")))?;
        let schema = zelox_iceberg::SchemaEvolver::assign_schema_field_ids(&schema)
...
fn columns_to_nested_fields(
    ...
    let temp_schema = zelox_iceberg::spec::Schema::builder()
        .with_fields(fields)
        .build()
        .map_err(|e| CatalogError::External(format!("Failed to build schema: {e}")))?;
    let assigned = zelox_iceberg::SchemaEvolver::assign_schema_field_ids(&temp_schema)
        .map_err(|e| CatalogError::External(format!("Failed to assign field ids: {e}")))?;
...
fn build_partition_spec(
    ...
    let mut partition_spec_builder = zelox_iceberg::PartitionSpec::builder();
        ...
        for column in &bucket_by.columns {
            if let Some(&source_id) = name_to_id.get(column) {
                partition_spec_builder = partition_spec_builder.add_field(
                    ...
                    zelox_iceberg::Transform::Bucket(num_buckets),
                );
            }
        }
    }
...
fn build_sort_order(
    ...
    for sort in sort_by {
        ...
        if let Some(&source_id) = name_to_id.get(&column) {
            sort_fields.push(zelox_iceberg::spec::sort::SortField {
                source_id,
                source_ids: vec![],
                transform,
                direction: if sort.ascending {
                    zelox_iceberg::spec::sort::SortDirection::Ascending
                } else {
                    zelox_iceberg::spec::sort::SortDirection::Descending
                },
                null_order: zelox_iceberg::spec::sort::NullOrder::Last, // TODO: Should this be configurable?
            });
        }
    }
    ...
    let order = zelox_iceberg::spec::sort::SortOrder {
        order_id: 1,
        fields: sort_fields,
    };
    ...
    Ok(Some(Box::new(crate::models::SortOrder {
        ...
        fields: order
            ...
            .map(|f| crate::models::SortField {
                source_id: f.source_id,
                transform: f.transform.to_string(),
                direction: if f.direction == zelox_iceberg::spec::sort::SortDirection::Ascending {
                    crate::models::SortDirection::Asc
                } else {
                    crate::models::SortDirection::Desc
                },
                null_order: if f.null_order == zelox_iceberg::spec::sort::NullOrder::First {
...
fn parse_sort_column(column: &str) -> CatalogResult<(String, zelox_iceberg::Transform)> {
    let column = column.trim();
    let Some((function, arguments)) = parse_transform_function(column) else {
        return Ok((column.to_string(), zelox_iceberg::Transform::Identity));
    };
    ...
    match function.to_ascii_lowercase().as_str() {
        "year" | "years" => {
            parse_unary_sort_transform(function, arguments, zelox_iceberg::Transform::Year)
        }
        "month" | "months" => {
            parse_unary_sort_transform(function, arguments, zelox_iceberg::Transform::Month)
        }
        "day" | "days" => {
            parse_unary_sort_transform(function, arguments, zelox_iceberg::Transform::Day)
        }
        "hour" | "hours" => {
            parse_unary_sort_transform(function, arguments, zelox_iceberg::Transform::Hour)
        }
        "bucket" => {
        ...
        "truncate" => {
            ...
            if let Some(width) = parse_positive_u32(first) {
                Ok((second.to_string(), zelox_iceberg::Transform::Truncate(width)))
            } else if let Some(width) = parse_positive_u32(second) {
                Ok((first.to_string(), zelox_iceberg::Transform::Truncate(width)))
            } else {
...
fn parse_unary_sort_transform(
    function: &str,
    arguments: Vec<&str>,
    transform: zelox_iceberg::Transform,
) -> CatalogResult<(String, zelox_iceberg::Transform)> {
