...
use zelox_catalog::error::CatalogError;
...
use zelox_common_datafusion::column_features::{ColumnFeatures, ColumnFeaturesBuilder};
...
use zelox_logical_plan::file_write::{FileWriteNode, FileWriteOptions};
...
impl PlanResolver<'_> {
    ...
    async fn rewrite_write_input_with_column_expressions(
        ...
        for col in &info.columns {
            ...
            let ast_expr =
                zelox_sql_analyzer::parser::parse_expression(gen_expr_str).map_err(|e| {
                    PlanError::invalid(format!(
                        "failed to parse generation expression `{gen_expr_str}`: {e}"
                    ))
                })?;
            let spec_expr =
                zelox_sql_analyzer::expression::from_ast_expression(ast_expr).map_err(|e| {
                    PlanError::invalid(format!(
