...
use zelox_catalog::provider::{CatalogPartitionField, CreateTemporaryViewOptions};
...
impl PlanResolver<'_> {
    ...
    pub(super) async fn resolve_command_write_stream(
        ...
        if format.eq_ignore_ascii_case("memory") {
            ...
            // through Zelox's CatalogManager (NOT `SessionContext::register_table`, which
            // targets DataFusion's default `datafusion` catalog that Zelox does not use —
        ...
        // strips. See `zelox_data_source::streaming_sink_log`.
        ...
        let enable_commit_log = |builder: WritePlanBuilder| {
            let builder = match &checkpoint {
                Some(cp) => builder.with_options(vec![(
                    zelox_common_datafusion::datasource::STREAM_CHECKPOINT_OPTION.to_string(),
                    cp.clone(),
                )]),
                None => builder,
            };
            match &realtime_interval {
                Some(iv) => builder.with_options(vec![(
                    zelox_common_datafusion::datasource::STREAM_REALTIME_INTERVAL_OPTION.to_string(),
                    iv.clone(),
                )]),
                None => builder,
            }
