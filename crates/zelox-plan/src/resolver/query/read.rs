...
use zelox_common_datafusion::catalog::{TableColumnStatus, TableKind};
use zelox_common_datafusion::datasource::{OptionLayer, SourceInfo, TableFormatRegistry};
use zelox_common_datafusion::extension::SessionExtensionAccessor;
use zelox_common_datafusion::literal::LiteralEvaluator;
use zelox_common_datafusion::rename::logical_plan::rename_logical_plan;
use zelox_common_datafusion::rename::table_provider::RenameTableProvider;
use zelox_common_datafusion::utils::items::ItemTaker;
use zelox_python_udf::udf::pyspark_unresolved_udf::PySparkUnresolvedUDF;
...
impl PlanResolver<'_> {
    ...
    pub(super) async fn resolve_query_read_dynamic_table(
        ...
        let name = zelox_sql_analyzer::expression::from_ast_object_name(
            zelox_sql_analyzer::parser::parse_object_name(&name_str)?,
        )?;
    ...
    async fn resolve_table_view(
        ...
        let ast = zelox_sql_analyzer::parser::parse_one_statement(&definition)?;
        let spec_plan = zelox_sql_analyzer::statement::from_ast_statement(ast)?;
