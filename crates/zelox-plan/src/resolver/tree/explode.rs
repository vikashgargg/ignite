...
use zelox_function::scalar::explode::{Explode, ExplodeKind};
    ...
    fn f_up(&mut self, node: Expr) -> Result<Transformed<Expr>> {
        ...
        let out = match (data_type, with_position, is_inline) {
            ...
            (ExplodeDataType::Map, false, _) => {
                vec![
                    ident(&name).field(ZELOX_MAP_KEY_FIELD_NAME).alias("key"),
                    ident(&name).field(ZELOX_MAP_VALUE_FIELD_NAME).alias("value"),
                ]
            }
            (ExplodeDataType::Map, true, _) => vec![
                ...
                ident(&name)
                    .field("col")
                    .field(ZELOX_MAP_KEY_FIELD_NAME)
                    .alias("key"),
                ident(&name)
                    .field("col")
                    .field(ZELOX_MAP_VALUE_FIELD_NAME)
                    .alias("value"),
            ],
        };
