...
pub(crate) struct SparkPartitionIdRewriter<'s> {
    plan: LogicalPlan,
    state: &'s mut PlanResolverState,
    column_name: Option<String>,
}

impl<'s> PlanRewriter<'s> for SparkPartitionIdRewriter<'s> {
...
impl TreeNodeRewriter for SparkPartitionIdRewriter<'_> {
    type Node = Expr;
        ...
        let col = match &self.column_name {
            ...
            None => {
                ...
                self.plan = LogicalPlan::Extension(Extension {
                    node: Arc::new(SparkPartitionIdNode::try_new(Arc::new(plan), col.clone())?),
                });
