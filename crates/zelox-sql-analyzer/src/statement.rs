...
use zelox_common::spec::QueryPlan;
use zelox_sql_parser::ast::expression::{BooleanLiteral, Expr, OrderDirection};
use zelox_sql_parser::ast::identifier::{Ident, ObjectName};
use zelox_sql_parser::ast::keywords::{Cascade, Global, Overwrite, Restrict, Temp, Temporary};
use zelox_sql_parser::ast::literal::{IntegerLiteral, NumberLiteral, StringLiteral};
use zelox_sql_parser::ast::operator::{Minus, Plus};
use zelox_sql_parser::ast::query::{IdentList, WhereClause};
use zelox_sql_parser::ast::statement::{
...
use zelox_sql_parser::tree::TreeText;
...
) -> SqlResult<Vec<spec::AlterTableColumnSpec>> {
    ...
    match operation {
        ...
        AlterTableOperation::DropColumns {
            ...
            let names = match names {
                ColumnDropList::Delimited { columns, .. } => columns,
        ...
        AlterTableOperation::RenameColumn { old, new, .. } => {
            Ok(spec::AlterTableOperation::RenameColumn {
                old: from_ast_object_name(old)?,
                new: from_ast_object_name(new)?,
            })
        }
