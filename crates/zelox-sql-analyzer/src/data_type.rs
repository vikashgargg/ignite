...
use zelox_common::spec::ARROW_DECIMAL128_MAX_PRECISION;
use zelox_sql_parser::ast::data_type::{
    DataType, IntervalDayTimeUnit, IntervalType, IntervalYearMonthUnit, StructField, TimezoneType,
};
use zelox_sql_parser::ast::literal::IntegerLiteral;
use zelox_sql_parser::ast::operator::{LeftParenthesis, RightParenthesis};
...
pub fn from_ast_data_type(sql_type: DataType) -> SqlResult<spec::DataType> {
    match sql_type {
        ...
        DataType::Geometry(_, _, srid, _) => {
            let srid = match srid {
                zelox_sql_parser::ast::data_type::GeometrySrid::Srid(lit) => lit.value as i32,
                zelox_sql_parser::ast::data_type::GeometrySrid::Any(_) => -1,
            };
            Ok(spec::DataType::Geometry { srid })
        }
        DataType::Geography(_, _, srid, _) => {
            let srid = match srid {
                zelox_sql_parser::ast::data_type::GeographySrid::Srid(lit) => lit.value as i32,
                zelox_sql_parser::ast::data_type::GeographySrid::Any(_) => -1,
            };
