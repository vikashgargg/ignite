...
fn generate_build_partial_options_impl(
    ...
    let fallback_arm = if has_any_arm && !all_arms_present {
        ...
        if option_list_arm.is_empty() {
            fallback_patterns.push(
                quote! { zelox_common_datafusion::datasource::OptionLayer::OptionList { .. } },
            );
        }
        if table_property_list_arm.is_empty() {
            fallback_patterns.push(
                quote! { zelox_common_datafusion::datasource::OptionLayer::TablePropertyList { .. } },
            );
        }
        if table_location_arm.is_empty() {
            fallback_patterns.push(
                quote! { zelox_common_datafusion::datasource::OptionLayer::TableLocation { .. } },
            );
        }
        if timestamp_arm.is_empty() {
            fallback_patterns.push(
                quote! { zelox_common_datafusion::datasource::OptionLayer::AsOfTimestamp { .. } },
            );
        }
        if integer_version_arm.is_empty() {
            fallback_patterns.push(
                quote! { zelox_common_datafusion::datasource::OptionLayer::AsOfIntegerVersion { .. } },
            );
        }
        if string_version_arm.is_empty() {
            fallback_patterns.push(
                quote! { zelox_common_datafusion::datasource::OptionLayer::AsOfStringVersion { .. } },
            );
        }
    ...
    Ok(quote! {
        impl crate::options::BuildPartialOptions<#partial_options_name>
            for zelox_common_datafusion::datasource::OptionLayer
        {
...
fn build_key_match_arm(
    ...
    let variant = match kind {
        OptionKeyKind::Option => {
            quote! { zelox_common_datafusion::datasource::OptionLayer::OptionList { items } }
        }
        OptionKeyKind::TableProperty => {
            quote! { zelox_common_datafusion::datasource::OptionLayer::TablePropertyList { items } }
        }
    };
    Ok(quote! {
        #variant => {
...
fn build_table_location_match_arm(
    ...
    Ok(quote! {
        zelox_common_datafusion::datasource::OptionLayer::TableLocation { value } => {
            #(#statements)*
        }
    })
}
...
fn build_temporal_clause_match_arm(
    ...
    let variant = match kind {
        TemporalClauseKind::AsOfTimestamp => {
            quote! { zelox_common_datafusion::datasource::OptionLayer::AsOfTimestamp { value } }
        }
        TemporalClauseKind::AsOfIntegerVersion => {
            quote! {
                zelox_common_datafusion::datasource::OptionLayer::AsOfIntegerVersion { value }
            }
        }
        TemporalClauseKind::AsOfStringVersion => {
            quote! {
                zelox_common_datafusion::datasource::OptionLayer::AsOfStringVersion { value }
            }
        }
    };
    Ok(quote! {
