...
///
/// Input:
/// - `__zelox_delta_replay_path` (Utf8): derived file path key
/// - `__zelox_delta_is_remove` (Boolean): derived marker for `remove(path)`
/// - `__zelox_delta_log_version` (Int64): derived log version from `_delta_log` filename prefix
/// - payload columns: any additional columns carried through for the winning `add` row
///
/// Notes:
/// - We replay actions in **newest-first** order (by `__zelox_delta_log_version`) within each
///   `__zelox_delta_replay_path`. The first action we observe for a path decides the final state
...
///   and sorted by `__zelox_delta_replay_path`, enabling streaming replay without materializing the
