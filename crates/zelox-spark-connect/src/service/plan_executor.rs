...
use zelox_common_datafusion::session::job::JobService;
use zelox_plan::{resolve_and_execute_plan, resolve_and_execute_plan_with_options};
...
pub(crate) async fn handle_execute_write_stream_operation_start(
    ...
    // docs/design/streaming-update-retraction-mode.md). `allowedLateness` (a Zelox writeStream
    ...
    // Zelox realtime mode: one long-lived unbounded pipeline (low latency) with per-epoch offset
    // commits. For RealTime the duration is the checkpoint/commit interval (4.2 semantics), same
    // knob as the continuous checkpoint interval. Zelox's realtime path is STATEFUL/windowed — a
...
pub(crate) async fn handle_execute_register_datasource(
    ...
    // Register in the session-scoped TableFormatRegistry with embedded pickled bytes
    {
        ...
        use zelox_data_source::formats::python::PythonTableFormat;
