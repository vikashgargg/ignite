...
pub async fn register_query(id: String, name: String) {
    registry().write().await.push(QueryEntry {
        id,
        name,
        is_active: true,
        batches_committed: 0,
        ...
        queries
            ...
            .map(|q| {
                format!(
                    ...
                    q.name,
                    if q.is_active {
                        "<span style='color:#2a2'>Active</span>"
                    } else {
...
<body>
<h1>⚡ Zelox Spark Engine <span class="badge up">Running</span></h1>
<div>
...
pub async fn serve(host: &str, port: u16) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    log::info!("Zelox Web UI at http://{addr}");
    let app = Router::new()
