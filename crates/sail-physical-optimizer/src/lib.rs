...
pub fn get_physical_optimizers(
    ...
    // DataFusion 54 added WindowTopN (Filter(row_number<=K)→Window→Sort collapses to a bounded
    // TopK), HashJoinBuffering, and TopKRepartition to its default pipeline — adopt them in the
    // same positions so we keep inheriting DataFusion's optimizer improvements.
