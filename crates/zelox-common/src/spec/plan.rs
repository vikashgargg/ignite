...
pub enum StreamTrigger {
    ...
    /// Routes to Zelox's realtime engine (long-lived unbounded pipeline, per-epoch commit);
    /// Zelox's realtime path is STATEFUL/windowed — a superset of 4.2's stateless-only RTM.
