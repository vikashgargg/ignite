...
/// enabled when distributed streaming is on (`ZELOX_DISTRIBUTED_STREAM=1`) — a cut streaming stage only
/// actually distributes if its partitions also spread — or explicitly via `ZELOX_EVEN_SPREAD=1`. Default
...
fn placement_policy() -> PlacementPolicy {
    ...
    *POLICY.get_or_init(|| {
        let on = std::env::var("ZELOX_DISTRIBUTED_STREAM").as_deref() == Ok("1")
            || std::env::var("ZELOX_EVEN_SPREAD").as_deref() == Ok("1");
        if on {
