...
use log::debug;
use prost::Message;
use zelox_common_datafusion::error::CommonErrorCause;
use zelox_data_source::formats::kafka::{KafkaSourceExec, ValueParseSpec};
use zelox_delta_lake::physical_plan::DeltaPhysicalExprAdapterFactory;
use zelox_function::scalar::json::from_json::SparkFromJsonOptions;
use zelox_python_udf::error::PyErrExtractor;
use zelox_server::actor::{Actor, ActorContext};
use zelox_telemetry::telemetry::global_metrics;
use zelox_telemetry::{trace_execution_plan, TracingExecOptions};
...
impl TaskRunner {
    ...
    /// VAJ-T7 source-fusion (opt-in `ZELOX_T7_FUSE`): when a `ProjectionExec` whose single output
    ...
    fn rewrite_source_fusion(
        ...
        // shape), so it is safe as a default. Opt OUT with `ZELOX_T7_FUSE=0` for A/B or debugging.
        if matches!(
            std::env::var("ZELOX_T7_FUSE").as_deref(),
