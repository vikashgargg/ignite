...
use prost::Message;
use zelox_catalog_system::physical_plan::SystemTableExec;
use zelox_common_datafusion::array::record_batch::{read_record_batches, write_record_batches};
use zelox_common_datafusion::catalog::{CatalogPartitionField, PartitionTransform};
use zelox_common_datafusion::datasource::PhysicalSinkMode;
use zelox_common_datafusion::system::catalog::SystemTable;
use zelox_common_datafusion::udf::StreamUDF;
use zelox_data_source::formats::binary::source::BinarySource;
use zelox_data_source::formats::console::ConsoleSinkExec;
use zelox_data_source::formats::json::permissive::{JsonMode, PermissiveJsonSource};
use zelox_data_source::formats::kafka::{
    KafkaReadOptions, KafkaSinkExec, KafkaSourceExec, ValueParseSpec,
};
use zelox_data_source::formats::python::{
    InputPartition, PythonDataSourceExec, PythonDataSourceWriteCommitExec,
    PythonDataSourceWriteExec,
};
use zelox_data_source::formats::rate::RateSourceExec;
use zelox_data_source::formats::socket::{SocketReadOptions, SocketSourceExec};
use zelox_data_source::formats::text::source::TextSource;
use zelox_data_source::formats::text::writer::{TextSink, TextWriterOptions};
use datafusion::execution::object_store::ObjectStoreUrl;
use object_store::path::Path as StorePath;
use zelox_data_source::formats::file_stream::FileSourceExec;
use zelox_data_source::streaming_decode::{
...
use zelox_data_source::options::gen::RateReadOptions;
use zelox_delta_lake::physical_plan::{
...
use zelox_delta_lake::spec::{Action, ColumnMappingMode, DeltaOperation, StructType};
use zelox_function::aggregate::bitmap_and_agg::BitmapAndAggFunction;
use zelox_function::scalar::json::from_json::SparkFromJsonOptions;
use zelox_function::aggregate::bitmap_construct_agg::BitmapConstructAggFunction;
use zelox_function::aggregate::bitmap_or_agg::BitmapOrAggFunction;
use zelox_function::aggregate::histogram_numeric::HistogramNumericFunction;
use zelox_function::aggregate::kurtosis::KurtosisFunction;
use zelox_function::aggregate::max_min_by::{MaxByFunction, MinByFunction};
use zelox_function::aggregate::mode::ModeFunction;
use zelox_function::aggregate::percentile::PercentileFunction;
use zelox_function::aggregate::percentile_disc::PercentileDisc;
use zelox_function::aggregate::product::ProductFunction;
use zelox_function::aggregate::schema_of_variant_agg::SchemaOfVariantAggFunction;
use zelox_function::aggregate::skewness::SkewnessFunc;
use zelox_function::aggregate::try_avg::TryAvgFunction;
use zelox_function::scalar::array::arrays_zip::ArraysZip;
use zelox_function::scalar::array::spark_array::SparkArray;
use zelox_function::scalar::array::spark_array_compact::SparkArrayCompact;
use zelox_function::scalar::array::spark_array_item_with_position::ArrayItemWithPosition;
use zelox_function::scalar::array::spark_array_min_max::{ArrayMax, ArrayMin};
use zelox_function::scalar::array::spark_sequence::SparkSequence;
use zelox_function::scalar::collection::spark_concat::SparkConcat;
use zelox_function::scalar::collection::spark_reverse::SparkReverse;
use zelox_function::scalar::csv::spark_from_csv::SparkFromCSV;
use zelox_function::scalar::csv::SparkSchemaOfCsv;
use zelox_function::scalar::datetime::convert_tz::ConvertTz;
use zelox_function::scalar::datetime::negate_duration::NegateDuration;
use zelox_function::scalar::datetime::spark_date::SparkDate;
use zelox_function::scalar::datetime::spark_date_trunc::SparkDateTrunc;
use zelox_function::scalar::datetime::spark_interval::{
    SparkCalendarInterval, SparkDayTimeInterval, SparkYearMonthInterval,
};
use zelox_function::scalar::datetime::spark_last_day::SparkLastDay;
use zelox_function::scalar::datetime::spark_make_time::SparkMakeTime;
use zelox_function::scalar::datetime::spark_make_timestamp::SparkMakeTimestampNtz;
use zelox_function::scalar::datetime::spark_make_ym_interval::SparkMakeYmInterval;
use zelox_function::scalar::datetime::spark_next_day::SparkNextDay;
use zelox_function::scalar::datetime::spark_time::SparkTime;
use zelox_function::scalar::datetime::spark_time_diff::SparkTimeDiff;
use zelox_function::scalar::datetime::spark_time_trunc::SparkTimeTrunc;
use zelox_function::scalar::datetime::spark_timestamp::SparkTimestamp;
use zelox_function::scalar::datetime::spark_to_chrono_fmt::SparkToChronoFmt;
use zelox_function::scalar::datetime::spark_try_make_timestamp_ntz::SparkTryMakeTimestampNtz;
use zelox_function::scalar::datetime::spark_try_to_timestamp::SparkTryToTimestamp;
use zelox_function::scalar::datetime::spark_unix_timestamp::SparkUnixTimestamp;
use zelox_function::scalar::datetime::spark_year::SparkYear;
use zelox_function::scalar::datetime::timestamp_now::TimestampNow;
use zelox_function::scalar::drop_struct_field::DropStructField;
use zelox_function::scalar::explode::{explode_name_to_kind, Explode};
use zelox_function::scalar::geo::st_asbinary::StAsBinary;
use zelox_function::scalar::geo::st_geogfromwkb::StGeogFromWKB;
use zelox_function::scalar::geo::st_geomfromwkb::StGeomFromWKB;
use zelox_function::scalar::hash::spark_murmur3_hash::SparkMurmur3Hash;
use zelox_function::scalar::hash::spark_xxhash64::SparkXxhash64;
use zelox_function::scalar::json::{SparkFromJson, SparkSchemaOfJson, SparkToJson};
use zelox_function::scalar::map::str_to_map::StrToMap;
use zelox_function::scalar::math::rand_poisson::RandPoisson;
use zelox_function::scalar::math::randn::Randn;
use zelox_function::scalar::math::random::Random;
use zelox_function::scalar::math::spark_abs::SparkAbs;
use zelox_function::scalar::math::spark_bin::SparkBin;
use zelox_function::scalar::math::spark_bround::SparkBRound;
use zelox_function::scalar::math::spark_ceil_floor::{SparkCeil, SparkFloor};
use zelox_function::scalar::math::spark_conv::SparkConv;
use zelox_function::scalar::math::spark_div::SparkIntervalDiv;
use zelox_function::scalar::math::spark_signum::SparkSignum;
use zelox_function::scalar::math::spark_try_add::SparkTryAdd;
use zelox_function::scalar::math::spark_try_div::SparkTryDiv;
use zelox_function::scalar::math::spark_try_mod::SparkTryMod;
use zelox_function::scalar::math::spark_try_mult::SparkTryMult;
use zelox_function::scalar::math::spark_try_subtract::SparkTrySubtract;
use zelox_function::scalar::math::spark_unhex::SparkUnHex;
use zelox_function::scalar::math::spark_uniform::SparkUniform;
use zelox_function::scalar::misc::raise_error::RaiseError;
use zelox_function::scalar::misc::spark_aes::{
    SparkAESDecrypt, SparkAESEncrypt, SparkTryAESDecrypt, SparkTryAESEncrypt,
};
use zelox_function::scalar::misc::version::SparkVersion;
use zelox_function::scalar::multi_expr::MultiExpr;
use zelox_function::scalar::predicate::rewrite_like_pattern::RewriteLikePatternFunc;
use zelox_function::scalar::spark_to_string::{SparkToLargeUtf8, SparkToUtf8, SparkToUtf8View};
use zelox_function::scalar::string::format_number::FormatNumber;
use zelox_function::scalar::string::levenshtein::Levenshtein;
use zelox_function::scalar::string::make_valid_utf8::MakeValidUtf8;
use zelox_function::scalar::string::randstr::Randstr;
use zelox_function::scalar::string::soundex::Soundex;
use zelox_function::scalar::string::spark_base64::{SparkBase64, SparkUnbase64};
use zelox_function::scalar::string::spark_concat_ws::SparkConcatWs;
use zelox_function::scalar::string::spark_encode_decode::{SparkDecode, SparkEncode};
use zelox_function::scalar::string::spark_mask::SparkMask;
use zelox_function::scalar::string::spark_quote::SparkQuote;
use zelox_function::scalar::string::spark_regexp_extract_all::SparkRegexpExtractAll;
use zelox_function::scalar::string::spark_sentences::SparkSentences;
use zelox_function::scalar::string::spark_split::SparkSplit;
use zelox_function::scalar::string::spark_to_binary::{SparkToBinary, SparkTryToBinary};
use zelox_function::scalar::string::spark_to_number::SparkToNumber;
use zelox_function::scalar::struct_function::StructFunction;
use zelox_function::scalar::update_struct_field::UpdateStructField;
use zelox_function::scalar::url::parse_url::ParseUrl;
use zelox_function::scalar::url::spark_try_parse_url::SparkTryParseUrl;
...
use zelox_function::scalar::xml::xpath::Xpath;
use zelox_function::scalar::xml::xpath_typed::{xpath_typed_name_to_kind, XpathTyped};
use zelox_function::window::SparkNtile;
use zelox_iceberg::physical_plan::{
...
use zelox_iceberg::IcebergWriterExecOptions;
use zelox_logical_plan::range::Range;
use zelox_logical_plan::show_string::{ShowStringFormat, ShowStringStyle};
use zelox_physical_plan::barrier::BarrierExec;
use zelox_physical_plan::catalog_command::CatalogCommandExec;
use zelox_physical_plan::map_partitions::MapPartitionsExec;
use zelox_physical_plan::merge_cardinality_check::MergeCardinalityCheckExec;
use zelox_physical_plan::monotonic_id::MonotonicIdExec;
use zelox_physical_plan::range::RangeExec;
use zelox_physical_plan::schema_pivot::SchemaPivotExec;
use zelox_physical_plan::show_string::ShowStringExec;
use zelox_physical_plan::spark_partition_id::SparkPartitionIdExec;
use zelox_physical_plan::repartition::ExplicitRepartitionExec;
use zelox_physical_plan::streaming::barrier_align::StreamBarrierAlignExec;
use zelox_physical_plan::streaming::collector::StreamCollectorExec;
use zelox_physical_plan::streaming::dedup::StreamDeduplicateExec;
use zelox_physical_plan::streaming::exchange::{StreamCoalesceExec, StreamExchangeExec};
use datafusion::physical_plan::aggregates::{AggregateExec, AggregateMode};
use datafusion::physical_plan::empty::EmptyExec;
use zelox_physical_plan::streaming::stream_join::StreamJoinExec;
use zelox_physical_plan::streaming::watermark::WatermarkExec;
use zelox_physical_plan::streaming::window_accum::WindowAccumExec;
use zelox_physical_plan::streaming::filter::StreamFilterExec;
use zelox_physical_plan::streaming::limit::StreamLimitExec;
use zelox_physical_plan::streaming::source_adapter::StreamSourceAdapterExec;
use zelox_python_udf::config::PySparkUdfConfig;
use zelox_python_udf::udf::pyspark_batch_collector::PySparkBatchCollectorUDF;
use zelox_python_udf::udf::pyspark_cogroup_map_udf::PySparkCoGroupMapUDF;
use zelox_python_udf::udf::pyspark_group_map_udf::{PySparkGroupMapMode, PySparkGroupMapUDF};
use zelox_python_udf::udf::pyspark_map_iter_udf::{PySparkMapIterKind, PySparkMapIterUDF};
use zelox_python_udf::udf::pyspark_udaf::{PySparkGroupAggKind, PySparkGroupAggregateUDF};
use zelox_python_udf::udf::pyspark_udf::{PySparkUDF, PySparkUdfKind};
...
impl PhysicalExtensionCodec for RemoteExecutionCodec {
    fn try_decode(
        ...
        match node_kind {
            ...
            NodeKind::DeltaScanByAdds(gen::DeltaScanByAddsExecNode {
                ...
                let scan_config: zelox_delta_lake::datasource::DeltaScanConfig =
                    if scan_config_json.is_empty() {
                        zelox_delta_lake::datasource::DeltaScanConfig::default()
                    } else {
            ...
            NodeKind::WindowAccum(gen::WindowAccumExecNode {
                ...
                let mode = if update_mode {
                    zelox_physical_plan::streaming::window_accum::WindowOutputMode::Update
                } else {
                    zelox_physical_plan::streaming::window_accum::WindowOutputMode::Append
                };
                Ok(Arc::new(
            ...
            NodeKind::IcebergManifestScan(gen::IcebergManifestScanExecNode {
                ...
                let snapshot: zelox_iceberg::spec::Snapshot = serde_json::from_str(&snapshot_json)
                    .map_err(|e| {
            ...
            NodeKind::CatalogCommand(gen::CatalogCommandExecNode { schema, command }) => {
                let schema = Arc::new(self.try_decode_schema(&schema)?);
                let command: zelox_catalog::command::CatalogCommand = serde_json::from_str(&command)
    ...
    fn try_encode(&self, node: Arc<dyn ExecutionPlan>, buf: &mut Vec<u8>) -> Result<()> {
        let node_kind = if let Some(range) = node.downcast_ref::<RangeExec>() {
            ...
            if let Some(file_scan) = source.downcast_ref::<FileScanConfig>() {
                    ...
                    file_source.downcast_ref::<BinarySource>()
                {
                    ...
                    file_source.downcast_ref::<PermissiveJsonSource>()
                {
                    let base_config = self.try_encode_message(serialize_file_scan_config(
                        file_scan,
                        self,
            ...
            node.downcast_ref::<DeltaLogReplayExec>()
        {
            ...
            NodeKind::ConsoleSink(gen::ConsoleSinkExecNode { input })
        } else if let Some(sink) = node.downcast_ref::<KafkaSinkExec>() {
            ...
            node.downcast_ref::<StreamBarrierAlignExec>()
        {
            let input = self.try_encode_plan(barrier_align.input().clone())?;
            NodeKind::StreamBarrierAlign(gen::StreamBarrierAlignExecNode { input })
        } else if let Some(coalesce) = node.downcast_ref::<StreamCoalesceExec>() {
            let input = self.try_encode_plan(coalesce.input().clone())?;
            NodeKind::StreamCoalesce(gen::StreamCoalesceExecNode { input })
        } else if let Some(exchange) = node.downcast_ref::<StreamExchangeExec>() {
        ...
        } else if let Some(decode) = node.downcast_ref::<FlowEventToDataExec>() {
            let input = self.try_encode_plan(decode.input().clone())?;
            NodeKind::FlowEventToData(gen::FlowEventToDataExecNode { input })
        } else if let Some(watermark) = node.downcast_ref::<WatermarkExec>() {
        ...
        } else if let Some(dedup) = node.downcast_ref::<StreamDeduplicateExec>() {
        ...
        } else if let Some(kafka) = node.downcast_ref::<KafkaSourceExec>() {
            let o = kafka.options();
        ...
        } else if let Some(sink) = node.downcast_ref::<RealtimeFileSinkExec>() {
            let input = self.try_encode_plan(sink.input().clone())?;
            NodeKind::RealtimeFileSink(gen::RealtimeFileSinkExecNode {
                input,
        ...
        } else if let Some(commit) = node.downcast_ref::<StreamingSinkCommitExec>() {
        ...
        } else if let Some(adapter) = node.downcast_ref::<EmptySinkAdapterExec>() {
            let input = self.try_encode_plan(adapter.input().clone())?;
            NodeKind::EmptySinkAdapter(gen::EmptySinkAdapterExecNode { input })
        } else if let Some(select) = node.downcast_ref::<PartitionSelectExec>() {
        ...
        } else if let Some(parallel) = node.downcast_ref::<ParallelStreamSinkExec>() {
            let children = parallel
                ...
                .collect::<Result<Vec<_>>>()?;
            NodeKind::ParallelStreamSink(gen::ParallelStreamSinkExecNode { children })
        } else if let Some(file_source) = node.downcast_ref::<FileSourceExec>() {
            let input = self.try_encode_plan(file_source.input().clone())?;
            NodeKind::FileSource(gen::FileSourceExecNode { input })
        } else if let Some(repart) = node.downcast_ref::<ExplicitRepartitionExec>() {
        ...
        } else if let Some(join) = node.downcast_ref::<StreamJoinExec>() {
    ...
    fn try_decode_udf(&self, name: &str, buf: &[u8]) -> Result<Arc<ScalarUDF>> {
        ...
        match name {
            ...
            "json_length" | "json_len" => Ok(zelox_function::scalar::json::json_length_udf()),
            "json_as_text" => Ok(zelox_function::scalar::json::json_as_text_udf()),
            "json_object_keys" | "json_keys" => {
                Ok(zelox_function::scalar::json::json_object_keys_udf())
            }
            ...
            "xpath" => Ok(Arc::new(ScalarUDF::from(
                zelox_function::scalar::xml::xpath::Xpath::new(),
            ))),
...
mod tests {
    ...
    fn test_round_trip_stream_barrier_align_exec() -> Result<()> {
        ...
        assert!(
            decoded.is::<StreamBarrierAlignExec>(),
    ...
    fn test_round_trip_streaming_shuffle_operators() -> Result<()> {
        ...
        assert!(decoded.is::<FlowEventToDataExec>(), "top = FlowEventToData");
        let coalesce_d = &decoded.children()[0];
        assert!(coalesce_d.is::<StreamCoalesceExec>(), "mid = StreamCoalesce");
    ...
    fn test_round_trip_kafka_source_exec() -> Result<()> {
        use datafusion::arrow::datatypes::{DataType, Field, Schema};
        use zelox_data_source::formats::kafka::KafkaReadOptions;
    ...
    fn test_round_trip_kafka_source_exec_fused() -> Result<()> {
        ...
        use zelox_data_source::formats::kafka::{KafkaReadOptions, ValueParseSpec};
        use zelox_function::scalar::json::from_json::SparkFromJsonOptions;
    ...
    fn test_round_trip_file_source_and_explicit_repartition() -> Result<()> {
        ...
        let decoded = codec.try_decode_plan(&buf, &TaskContext::default())?;
        assert!(decoded.is::<FileSourceExec>(), "FileSourceExec round-trips");
