...
impl FlightService for TaskStreamFlightServer {
    ...
    async fn do_get(
        ...
        // (this Flight path); the in-process exchange is untouched (ZELOX_SHUFFLE_BATCH_ROWS=0 disables).
        let target = zelox_physical_plan::streaming::exchange::shuffle_batch_rows();
        let stream: TaskStreamSource = if target > 1 {
            Box::pin(zelox_physical_plan::streaming::exchange::coalesce_flow_events(
                stream, target,
            ))
        } else {
            stream
