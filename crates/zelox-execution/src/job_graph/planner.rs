...
impl JobGraph {
    pub fn try_new(plan: Arc<dyn ExecutionPlan>) -> ExecutionResult<Self> {
        ...
        let distributed_stream = std::env::var("ZELOX_DISTRIBUTED_STREAM").as_deref() == Ok("1");
...
///
/// Opt-in via `ZELOX_DISTRIBUTED_STREAM=1` — the default keeps the in-process `StreamExchangeExec`
