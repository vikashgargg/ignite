...
use zelox_common::config::CatalogCacheConfig;
use zelox_common_datafusion::catalog::{DatabaseStatus, TableStatus};
...
impl<P: CatalogProvider + ?Sized> CachingCatalogProvider<P> {
    pub fn new(
        ...
        let database_cache = match config.database_cache_type {
            zelox_common::config::CacheType::None => None,
            zelox_common::config::CacheType::Global => global_bundle
                .as_ref()
                .and_then(|b| b.database_cache.clone()),
            zelox_common::config::CacheType::Session => {
        ...
        let table_cache = match config.table_cache_type {
            zelox_common::config::CacheType::None => None,
            zelox_common::config::CacheType::Global => {
...
mod tests {
    ...
    use zelox_common_datafusion::catalog::{DatabaseStatus, TableKind, TableStatus};
    ...
    async fn test_caching_behavior() {
        ...
        let config = CatalogCacheConfig {
            database_cache_type: zelox_common::config::CacheType::Session,
            database_cache_size: Some(10),
            database_cache_ttl_secs: Some(60),
            table_cache_type: zelox_common::config::CacheType::Session,
            table_cache_size: Some(10),
            table_cache_ttl_secs: Some(60),
            view_cache_type: zelox_common::config::CacheType::Session,
