...
use zelox_common_datafusion::array::serde::ArrowSerializer;
    ...
    pub async fn execute<C: SessionExtensionAccessor>(
        ...
        let batch = match self {
            ...
            | CatalogCommand::RefreshByPath { .. } => {
                // No-op: Zelox does not maintain an in-memory table cache.
                display.empty().to_record_batch(vec![])?
            }
        };
        Ok(batch)
...
mod tests {
    ...
    use zelox_common_datafusion::catalog::display::{CatalogObjectDisplay, DefaultCatalogDisplay};
    use zelox_common_datafusion::catalog::{
        DatabaseStatus, TableColumnStatus, TableKind, TableStatus,
    };
    use zelox_common_datafusion::datasource::{SinkInfo, SourceInfo, TableFormat};
    use zelox_common_datafusion::session::plan::{PlanFormatter, PlanService};
    use serde::{Deserialize, Serialize};
    ...
    impl CatalogProvider for TestProvider {
        ...
        ) -> CatalogResult<zelox_common_datafusion::catalog::DatabaseStatus> {
            unreachable!()
        }
        ...
        ) -> CatalogResult<Vec<zelox_common_datafusion::catalog::DatabaseStatus>> {
            unreachable!()
        }
