...
use zelox_common::spec::{
    self, ZELOX_LIST_FIELD_NAME, ZELOX_MAP_FIELD_NAME, ZELOX_MAP_KEY_FIELD_NAME,
    ZELOX_MAP_VALUE_FIELD_NAME,
};
use zelox_common_datafusion::utils::datetime::localize_with_fallback;
use zelox_sql_analyzer::data_type::from_ast_data_type;
use zelox_sql_analyzer::parser as zelox_parser;
...
impl ScalarUDFImpl for SparkFromJson {
    ...
    fn invoke_with_args(&self, args: ScalarFunctionArgs) -> Result<ColumnarValue> {
        ...
        // Throughput attribution (ZELOX_WM_PROF): time the parse to gauge from_json's share of the
        // streaming gap (serde_json Value-per-row). Zero cost when the env var is unset.
        let _t = zelox_common_datafusion::streaming::event::encoding::wm_prof_enabled()
        ...
        if let Some(t) = _t {
            zelox_common_datafusion::streaming::event::encoding::FROM_JSON_NS.fetch_add(
...
fn parse_schema_to_data_type(schema: &str, session_timezone: &str) -> Result<DataType> {
    ...
    let ast = zelox_parser::parse_data_type(schema).or_else(|_| {
        // If that fails, treat as a bare field list and wrap in STRUCT<...>.
        zelox_parser::parse_data_type(&format!("STRUCT<{schema}>"))
    });
            ...
            match type_str {
                ...
                "map" => {
                    ...
                    let fields = Fields::from(vec![
                        Arc::new(Field::new(ZELOX_MAP_KEY_FIELD_NAME, key_dt, false)),
                        Arc::new(Field::new(
                            ZELOX_MAP_VALUE_FIELD_NAME,
                            value_dt,
                            value_contains_null,
...
fn json_type_name_to_data_type(type_name: &str, session_timezone: &str) -> Result<DataType> {
    match type_name {
        ...
        other => {
            ...
            let ast = zelox_parser::parse_data_type(other).map_err(|_| {
...
fn find_key_index(options: &MapArray, search_key: &str) -> Option<usize> {
    options
        .entries()
        .column_by_name(ZELOX_MAP_KEY_FIELD_NAME)
...
fn find_key_value(options: &MapArray, search_key: &str) -> Option<String> {
    if let Some(index) = find_key_index(options, search_key) {
        options
            .entries()
            .column_by_name(ZELOX_MAP_VALUE_FIELD_NAME)
            .and_then(|x| x.as_any().downcast_ref::<StringArray>())
            .and_then(|values| {
