...
pub struct HofEncodeParts {
    ...
    /// Only present for `zelox_array_aggregate` (the optional finish lambda).
...
impl ScalarUDFImpl for ZeloxArrayFilter {
    ...
    fn invoke_with_args(&self, args: ScalarFunctionArgs) -> Result<ColumnarValue> {
        let array = args
            ...
            .next()
            .ok_or_else(|| {
                datafusion_common::DataFusionError::Execution("zelox_array_filter: no args".into())
            })?
            .into_array(1)?;
        match array.data_type() {
            ...
            other => exec_err!("zelox_array_filter: unsupported type {:?}", other),
        }
    }
}
...
impl ScalarUDFImpl for ZeloxArrayTransform {
    ...
    fn invoke_with_args(&self, args: ScalarFunctionArgs) -> Result<ColumnarValue> {
        let array = args
            ...
            .ok_or_else(|| {
                datafusion_common::DataFusionError::Execution(
                    "zelox_array_transform: no args".into(),
                )
            })?
            .into_array(1)?;
        let result_field = match &self.return_type {
            ...
            _ => {
                return exec_err!(
                    "zelox_array_transform: unexpected return type {:?}",
                    self.return_type
                )
            }
        };
        match array.data_type() {
            ...
            other => exec_err!("zelox_array_transform: unsupported type {:?}", other),
        }
    }
}
...
impl ScalarUDFImpl for ZeloxArrayExists {
    ...
    fn invoke_with_args(&self, args: ScalarFunctionArgs) -> Result<ColumnarValue> {
        let array = args
            ...
            .next()
            .ok_or_else(|| {
                datafusion_common::DataFusionError::Execution("zelox_array_exists: no args".into())
            })?
            .into_array(1)?;
        match array.data_type() {
            DataType::List(_) => {
            ...
            other => exec_err!("zelox_array_exists: unsupported type {:?}", other),
        }
    }
}
...
impl ScalarUDFImpl for ZeloxArrayForAll {
    ...
    fn invoke_with_args(&self, args: ScalarFunctionArgs) -> Result<ColumnarValue> {
        let array = args
            ...
            .next()
            .ok_or_else(|| {
                datafusion_common::DataFusionError::Execution("zelox_array_forall: no args".into())
            })?
            .into_array(1)?;
        match array.data_type() {
            DataType::List(_) => {
            ...
            other => exec_err!("zelox_array_forall: unsupported type {:?}", other),
        }
    }
}
...
impl ScalarUDFImpl for ZeloxArrayAggregate {
    ...
    fn invoke_with_args(&self, args: ScalarFunctionArgs) -> Result<ColumnarValue> {
        ...
        let list_cv = iter.next().ok_or_else(|| {
            datafusion_common::DataFusionError::Execution("zelox_array_aggregate: no args".into())
        })?;
        let init_cv = iter.next().ok_or_else(|| {
            datafusion_common::DataFusionError::Execution(
                "zelox_array_aggregate: missing init arg".into(),
            )
        })?;
        ...
        let result = match list_arr.data_type() {
            ...
            other => return exec_err!("zelox_array_aggregate: unsupported type {:?}", other),
        };
...
impl ScalarUDFImpl for ZeloxArrayZipWith {
    ...
    fn invoke_with_args(&self, args: ScalarFunctionArgs) -> Result<ColumnarValue> {
        ...
        let arr1 = iter
            .next()
            .ok_or_else(|| {
                datafusion_common::DataFusionError::Execution("zelox_array_zip_with: no args".into())
            })?
            .into_array(1)?;
        let arr2 = iter
            .next()
            .ok_or_else(|| {
                datafusion_common::DataFusionError::Execution(
                    "zelox_array_zip_with: missing second arg".into(),
                )
            })?
            .into_array(arr1.len())?;
        ...
        let result_field = match &self.return_type {
            ...
            _ => {
                return exec_err!(
                    "zelox_array_zip_with: unexpected return type {:?}",
                    self.return_type
                )
            }
        };
        ...
        match (arr1.data_type(), arr2.data_type()) {
            ...
            (t1, t2) => exec_err!(
                "zelox_array_zip_with: mismatched or unsupported types {:?} {:?}",
                t1,
                t2
            ),
        }
...
fn array_zip_with_generic<O: OffsetSizeTrait>(
    ...
    if list1.len() != list2.len() {
        return exec_err!("zelox_array_zip_with: arrays must have same row count");
    }
...
impl ScalarUDFImpl for ZeloxArraySort {
    ...
    fn invoke_with_args(&self, args: ScalarFunctionArgs) -> Result<ColumnarValue> {
        let array = args
            ...
            .next()
            .ok_or_else(|| {
                datafusion_common::DataFusionError::Execution("zelox_array_sort: no args".into())
            })?
            .into_array(1)?;
        match array.data_type() {
            DataType::List(_) => {
            ...
            other => exec_err!("zelox_array_sort: unsupported type {:?}", other),
        }
    }
}
...
impl ScalarUDFImpl for ZeloxMapTransformKeys {
    ...
    fn invoke_with_args(&self, args: ScalarFunctionArgs) -> Result<ColumnarValue> {
        let array = args
            ...
            .ok_or_else(|| {
                datafusion_common::DataFusionError::Execution(
                    "zelox_map_transform_keys: no args".into(),
                )
            })?
            .into_array(1)?;
        let map = array.as_any().downcast_ref::<MapArray>().ok_or_else(|| {
            datafusion_common::DataFusionError::Execution(
                "zelox_map_transform_keys: expected MapArray".into(),
            )
        })?;
...
impl ScalarUDFImpl for ZeloxMapTransformValues {
    ...
    fn invoke_with_args(&self, args: ScalarFunctionArgs) -> Result<ColumnarValue> {
        let array = args
            ...
            .ok_or_else(|| {
                datafusion_common::DataFusionError::Execution(
                    "zelox_map_transform_values: no args".into(),
                )
            })?
            .into_array(1)?;
        let map = array.as_any().downcast_ref::<MapArray>().ok_or_else(|| {
            datafusion_common::DataFusionError::Execution(
                "zelox_map_transform_values: expected MapArray".into(),
            )
        })?;
...
impl ScalarUDFImpl for ZeloxMapFilter {
    ...
    fn invoke_with_args(&self, args: ScalarFunctionArgs) -> Result<ColumnarValue> {
        let array = args
            ...
            .next()
            .ok_or_else(|| {
                datafusion_common::DataFusionError::Execution("zelox_map_filter: no args".into())
            })?
            .into_array(1)?;
        let map = array.as_any().downcast_ref::<MapArray>().ok_or_else(|| {
            datafusion_common::DataFusionError::Execution(
                "zelox_map_filter: expected MapArray".into(),
            )
        })?;
...
impl ScalarUDFImpl for ZeloxMapZipWith {
    ...
    fn invoke_with_args(&self, args: ScalarFunctionArgs) -> Result<ColumnarValue> {
        ...
        let arr1 = args_iter
            ...
            .ok_or_else(|| {
                datafusion_common::DataFusionError::Execution(
                    "zelox_map_zip_with: missing first arg".into(),
                )
            })?
            .into_array(1)?;
        let arr2 = args_iter
            ...
            .ok_or_else(|| {
                datafusion_common::DataFusionError::Execution(
                    "zelox_map_zip_with: missing second arg".into(),
                )
            })?
            .into_array(1)?;
        let map1 = arr1.as_any().downcast_ref::<MapArray>().ok_or_else(|| {
            datafusion_common::DataFusionError::Execution(
                "zelox_map_zip_with: expected MapArray for arg1".into(),
            )
        })?;
        let map2 = arr2.as_any().downcast_ref::<MapArray>().ok_or_else(|| {
            datafusion_common::DataFusionError::Execution(
                "zelox_map_zip_with: expected MapArray for arg2".into(),
            )
        })?;
        ...
        let result_val_arr = if result_vals.is_empty() {
            let val_type = match &self.return_type {
                DataType::Map(f, _) => match f.data_type() {
                    ...
                    _ => return exec_err!("zelox_map_zip_with: invalid return type"),
                },
                _ => return exec_err!("zelox_map_zip_with: invalid return type"),
            };
