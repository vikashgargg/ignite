...
//!
//! When `VAJRA_PYTHON` is set (installed by `install.sh` to the venv's
...
/// Returns `true` if `VAJRA_PYTHON` is set (subprocess mode enabled).
pub fn is_subprocess_mode() -> bool {
    std::env::var_os("VAJRA_PYTHON").is_some()
}
...
impl UdfWorker {
    /// Spawn a new worker subprocess using `$VAJRA_PYTHON`.
    pub fn spawn() -> PyUdfResult<Self> {
        use std::io::Write as _;

        let python = std::env::var("VAJRA_PYTHON")
            .map_err(|_| PyUdfError::internal("VAJRA_PYTHON not set"))?;
        ...
        let mut child = Command::new(&python)
            ...
            .env("VAJRA_SPARK_PY", &spark_py_path)
            .spawn()
