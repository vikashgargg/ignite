"""
Vajra UDF Worker — standalone Python subprocess for executing PySpark UDFs.
...
# spawning the worker (written to a temp file referenced via VAJRA_SPARK_PY).
# Alternatively, we accept the source on the env var VAJRA_SPARK_PY_SRC.
# ---------------------------------------------------------------------------
...
def _load_spark_module() -> None:
    """Load spark.py from the path in VAJRA_SPARK_PY env var."""
    spark_py_path = os.environ.get("VAJRA_SPARK_PY")
    if spark_py_path and os.path.exists(spark_py_path):
        ...
        try:
            ...
            raise RuntimeError(
                "Cannot locate spark.py. Set VAJRA_SPARK_PY to its path."
            )
