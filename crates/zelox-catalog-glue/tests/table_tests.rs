...
async fn test_create_table() {
    ...
    match &existing_table.kind {
        zelox_common_datafusion::catalog::TableKind::Table { comment, .. } => {
            assert_eq!(comment, &Some("Product catalog table".to_string()));
        }
        _ => panic!("Expected Table kind"),
    }
...
async fn test_hive_rejects_transforms() {
    ...
    assert!(
        matches!(err, zelox_catalog::error::CatalogError::NotSupported(_)),
...
async fn test_iceberg_requires_location() {
    ...
    let err = result.unwrap_err();
    assert!(
        matches!(err, zelox_catalog::error::CatalogError::InvalidArgument(_)),
