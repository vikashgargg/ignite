...
use zelox_function::scalar::json::from_json::{parse_json_binary_to_struct, SparkFromJsonOptions};
...
/// the i32-offset safety. `ZELOX_SOURCE_MAX_BATCH_BYTES`. docs/design/streaming-memory-boundedness.md.
fn max_batch_bytes() -> usize {
    static V: std::sync::OnceLock<usize> = std::sync::OnceLock::new();
    *V.get_or_init(|| {
        std::env::var("ZELOX_SOURCE_MAX_BATCH_BYTES")
            .ok()
...
/// users can still override. Zelox's bounded catch-up read needs to saturate the broker (Flink
...
/// by `ZELOX_KAFKA_STATS` (sets `statistics.interval.ms`); prod-grade observability (Flink-equivalent
...
fn apply_consumer_throughput_defaults(cfg: &mut ClientConfig) {
    if std::env::var("ZELOX_KAFKA_STATS").is_ok() {
    ...
    // memory. `ZELOX_KAFKA_PREFETCH_KBYTES` (default 65536 = 64 MiB/partition; set 1048576 for the old
    // 1 GiB throughput-max). docs/design/streaming-memory-boundedness.md.
    let prefetch_kbytes = std::env::var("ZELOX_KAFKA_PREFETCH_KBYTES")
        .ok()
        ...
        .unwrap_or(65536); // 64 MiB/partition (was 1 GiB) — DEFENSIVE bound. MEASURED EKS 2026-07-02: for a
                           ...
                           // env-tunable (ZELOX_KAFKA_STATS logs the actual fetchq_size for observability).
    let prefetch_msgs = std::env::var("ZELOX_KAFKA_PREFETCH_MSGS")
        .ok()
        .and_then(|s| s.parse::<u64>().ok())
        .filter(|&n| n >= 1000)
...
impl StreamSource for KafkaStreamSource {
    ...
    async fn scan(
        ...
        // docs/design/continuous-stateful-eo-fix.md. `ZELOX_RT_SINGLE` opts out to the legacy
        ...
        // path (EO-complete, correctness_gate C6/C7). The `ZELOX_RT_SINGLE` opt-out to the legacy
    ...
    fn execute(
        ...
        if self.bounded {
            ...
            let events = async_stream::stream! {
                ...
                // Stats context logs the librdkafka prefetch queue bytes (ZELOX_KAFKA_STATS) — direct
                ...
                // FLIP-27 batch-queue path (gated ZELOX_KAFKA_BATCH_QUEUE): spawn one dedicated
                ...
                if batch_queue_enabled() {
                        ...
                        match item {
                            ReaderEvent::Batch(pb) => {
                                if zelox_common_datafusion::streaming::event::encoding::wm_prof_enabled() {
                                    zelox_common_datafusion::streaming::event::encoding::reader_inflight_account(
                    ...
                    yield Ok(FlowEvent::Marker(zelox_common_datafusion::streaming::event::marker::FlowMarker::EndOfData));
                    return;
                }
                ...
                // (the throughput path). `ZELOX_KAFKA_LEGACY_POLL=1` forces the old
                // one-timer-per-message poll + small batch — a kill-switch / A-B measurement lever.
                let fast_drain =
                    std::env::var("ZELOX_KAFKA_LEGACY_POLL").ok().as_deref() != Some("1");
                ...
                while remaining(&next) > 0 {
                    // Throughput attribution (ZELOX_WM_PROF): time the per-batch read (poll + append +
                    // finish) so the EKS profile pinpoints the source-read share. Zero cost when unset.
                    let _rd = zelox_common_datafusion::streaming::event::encoding::wm_prof_enabled()
                    ...
                    if builders.len() > 0 {
                        if let Some(t) = _rd {
                            use zelox_common_datafusion::streaming::event::encoding as prof;
        ...
        if let Some(interval_ms) = self.realtime_interval_ms {
            ...
            let events = async_stream::stream! {
                ...
                // FLIP-27 batch-queue continuous path (gated ZELOX_KAFKA_BATCH_QUEUE): one dedicated
                ...
                let idle_src = format!("kafka:{inst}");
                let mut idle_signaled = false;
                // Source-read CPU attribution (ZELOX_WM_PROF): the continuous path is event-driven, so
                ...
                let prof = zelox_common_datafusion::streaming::event::encoding::wm_prof_enabled();
                        ...
                        _ = timer.tick() => {
                            ...
                            if prof && read_acc > 0 {
                                zelox_common_datafusion::streaming::event::encoding::prof_add(
                                    &zelox_common_datafusion::streaming::event::encoding::SOURCE_READ_NS,
...
/// FLIP-27 batch-queue Kafka consume (gated `ZELOX_KAFKA_BATCH_QUEUE`, default OFF). The default
...
fn batch_queue_enabled() -> bool {
    std::env::var("ZELOX_KAFKA_BATCH_QUEUE").as_deref() == Ok("1")
}
...
mod bench {
    ...
    /// ZELOX_KAFKA_BATCH_QUEUE path) end-to-end — one reader thread per partition, shared channel,
