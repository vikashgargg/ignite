...
use zelox_common_datafusion::streaming::event::encoding::DecodedFlowEventStream;
use zelox_common_datafusion::streaming::event::marker::FlowMarker;
use zelox_common_datafusion::streaming::event::stream::FlowEventStream;
...
pub struct KafkaSinkExec {
    ...
    /// per-task `transactional.id` (`zelox-sink-<topic>-<idx>`) so parallel EO producers each fence their
    ...
    fn execute(
        ...
        let out = async_stream::stream! {
            ...
            if exactly_once {
                ...
                cfg.set("transactional.id", format!("zelox-sink-{topic}-{sink_partition}"));
