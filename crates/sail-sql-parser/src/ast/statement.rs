...
use crate::ast::keywords::{
    ...
    Functions, Generated, Global, If, In, Index, Inpath, Inputformat, Insert, Into, Is, Items,
    Keys, Lazy, Like, Lines, Load, Local, Location, Logical, Map, Matched, Merge, Name, Namespace,
    Namespaces, Noscan, Not, Null, On, Options, Or, Outputformat, Overwrite, Partition,
    Partitioned, Partitions, Primary, Properties, Purge, Recover, Refresh, Rename, Replace,
    Restrict, Row, Schema, Schemas, Serde, Serdeproperties, Set, Show, Skewed, Sorted, Source,
    Statistics, Stored, Table, Tables, Target, Tblproperties, Temp, Temporary, Terminated, Then,
    Time, To, Type, Uncache, Unique, Unset, Update, Use, Using, Values, Verbose, View, Views, When,
    With, Zone,
};
...
pub enum ColumnDefinitionOption {
    ...
    GeneratedAlwaysIdentity(
        ...
        Option<(
            LeftParenthesis,
            Vec<(Ident, Either<By, With>, Option<Minus>, NumberLiteral)>,
            RightParenthesis,
        )>,
    ),
...
pub enum CreateTableClause {
    ...
    // STORED BY 'classname' — Hive storage handler; silently accepted and ignored
    StoredBy(Stored, By, StringLiteral),
    Location(Location, StringLiteral),
    Comment(Comment, StringLiteral),
    Options(Options, PropertyList),
    Properties(Tblproperties, PropertyList),
    // DEFAULT COLLATION name — Spark 4.x; silently accepted and ignored
    DefaultCollation(Default, Ident, Ident),
    // SKEWED BY (cols) ON (values) — Hive DDL hint; silently accepted and ignored
    SkewedBy(
        Skewed,
        By,
        LeftParenthesis,
        Sequence<Ident, Comma>,
        RightParenthesis,
        On,
        LeftParenthesis,
        Sequence<Ident, Comma>,
...
pub enum AlterTableOperation {
    ...
    AlterColumn {
        ...
        items: Sequence<AlterColumnItem, Comma>,
    },
    ReplaceColumns {
        replace: Replace,
        columns: Either<Column, Columns>,
    ...
    // PARTITION (...) CHANGE COLUMN name new_name type [options] — HiveQL partition-scoped change
    ChangeColumnAfterPartition {
        #[parser(function = |(e, _), o| compose(e, o))]
        partition: PartitionClause,
        change: Change,
        column: Column,
    ...
    // PARTITION (...) REPLACE COLUMNS (...) — HiveQL partition-scoped replace
    ReplaceColumnsAfterPartition {
        #[parser(function = |(e, _), o| compose(e, o))]
        partition: PartitionClause,
        replace: Replace,
        columns: Either<Column, Columns>,
    ...
    // DEFAULT COLLATION name — Spark 4.x collation; silently accepted and ignored
    DefaultCollation(Default, Ident, Ident),
}
...
pub enum AlterColumnOperation {
    Type(
        Type,
        #[parser(function = |(_, d), _| d)] DataType,
        // optional trailing COMMENT / FIRST / AFTER — silently ignored
        #[parser(function = |(e, _), o| compose(e, o))] Vec<ColumnAlterationOption>,
    ),
    Comment(Comment, StringLiteral),
...
/// One item in a multi-column ALTER COLUMN list: `name operation`
#[derive(Debug, Clone, TreeParser, TreeSyntax, TreeText)]
#[parser(dependency = "(Expr, DataType)")]
pub struct AlterColumnItem {
    pub name: ObjectName,
    #[parser(function = |(e, d), o| compose((e, d), o))]
    pub operation: AlterColumnOperation,
}
...
pub enum AnalyzeTableModifier {
    ...
    ForAllColumns(
        ...
        All,
        Option<Columns>,
        Option<Sequence<ObjectName, Comma>>,
    ),
