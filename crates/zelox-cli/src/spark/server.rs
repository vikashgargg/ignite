...
use zelox_execution::worker_manager::leader_election::{
    KubernetesLeaderElector, LeaderElectionConfig,
};
use zelox_spark_connect::entrypoint::serve;
    ...
    F: Future<Output = Result<(), Box<dyn std::error::Error>>>,
{
    ...
    let (server_address, server_task) = runtime.handle().primary().block_on(async {
        ...
        let server_task = async move {
            info!("Zelox ready on {server_address} (Spark Connect gRPC) [mode: {mode_tag}]");
        ...
        elector
            .run_as_leader(|| {
                ...
                async move {
                    ...
                    info!("Zelox ready on {server_address} (Spark Connect gRPC) [mode: kubernetes-cluster-ha]");
...
/// `ZELOX_MODE` says.  `workers == 0` keeps the config-file default.
