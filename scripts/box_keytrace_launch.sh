...
N="${N:-100000000}"; REGION="${REGION:-ap-south-1}"; ITYPE="${INSTANCE_TYPE:-c7g.4xlarge}"  # 8 vCPU / 64 GiB — Kafka+Vajra headroom for 100M
...
KEY=/tmp/zelox-kt-key.pem; KN="zelox-kt-$$"
...
IID="$(aws ec2 run-instances --region "$REGION" --image-id "$AMI" --instance-type "$ITYPE" --key-name "$KN" \
  ...
  --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=zelox-kt}]' --query 'Instances[0].InstanceId' --output text)"
...
# On-box: build strip=false prof image, run the 100M realtime confidence, report. (see scripts/box_keytrace_onbox.sh)
scp -o StrictHostKeyChecking=no -i "$KEY" scripts/box_keytrace_onbox.sh ec2-user@"$IP":~/zelox/scripts/
ssh $SSHO "cd ~/zelox && sg docker -c 'N=$N bash scripts/box_keytrace_onbox.sh'" 2>&1 | mask
