...
BOOT=localhost:9092; BIN="$ROOT/target/debug/zelox"; PY="$ROOT/.venvs/smoke/bin/python"
...
# MinIO up + bucket
docker ps --format '{{.Names}}' | grep -qi zelox_minio || docker run -d --name zelox_minio -p 9000:9000 -p 9001:9001 \
...
run() { # $1=rows $2=label
  pkill -9 -f 'target/debug/zelox' 2>/dev/null; sleep 2
  mc "mc rm -r --force l/zelox/$2 l/zelox/${2}_ck"
  ZELOX_DISTRIBUTED_STREAM=1 ZELOX_WM_PROF=1 ZELOX_COMPLETE_ON_END=1 ZELOX_SHUFFLE_BATCH_ROWS="$1" \
  ...
  local R; R=$(SPARK_REMOTE="sc://localhost:$PORT" BOOT="$BOOT" TOPIC="$TOPIC" N_EVENTS="$N" MAXOFFSETS=1000000 \
    OUT="s3://zelox/$2" CK="s3://zelox/${2}_ck" timeout 300 "$PY" scripts/stream_windowed_agg.py 2>&1 | grep -aoE 'ZELOX_WAGG.*' | tail -1)
  ...
  pkill -9 -f 'target/debug/zelox' 2>/dev/null
  echo "  [$2 rows=$1] $R"
