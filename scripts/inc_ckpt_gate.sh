...
#   (a) EXACTLY-ONCE across a HARD crash (kill -9) with VAJRA_INC_CKPT=1 + a small spill budget — the
...
#       both for comparison.
# Requires: docker `vajra_kafka` up; target/debug/vajra (current); .venvs/smoke python.
...
[ -n "$KPOD" ] || { echo "FATAL: no kafka container (start vajra_kafka)"; exit 2; }
