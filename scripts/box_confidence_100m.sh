...
IID="$(aws ec2 run-instances --region "$REGION" --image-id "$AMI" --instance-type "$ITYPE" --key-name "$KN" \
  ...
  --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=zelox-confidence}]' --query 'Instances[0].InstanceId' --output text)"
...
# On-box: build strip=false prof image, run the 100M realtime confidence, report. (see scripts/box_onbox_confidence.sh)
scp -o StrictHostKeyChecking=no -i "$KEY" scripts/box_onbox_confidence.sh ec2-user@"$IP":~/zelox/scripts/
ssh $SSHO "cd ~/zelox && sg docker -c 'N=$N bash scripts/box_onbox_confidence.sh'" 2>&1 | mask
