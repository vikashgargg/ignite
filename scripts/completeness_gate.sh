...
# MAX_WATERMARK) — including the last boundary window whose end > max-event-time. EKS measured Vajra 9
...
echo "=== vajra server --workers $WORKERS (VAJRA_COMPLETE_ON_END=1 = bounded-complete / Flink-parity) ==="
pkill -9 -f 'target/debug/vajra' 2>/dev/null; sleep 1
RUST_LOG=warn VAJRA_COMPLETE_ON_END=1 "$BIN" server --ip 127.0.0.1 --port "$PORT" --mode local-cluster --workers "$WORKERS" >/tmp/comp_server.log 2>&1 &
...
SPARK_REMOTE="sc://localhost:$PORT" BOOT=localhost:9092 TOPIC=$TOPIC N_EVENTS=$N OUT=$OUT CK=$CK \
  "$PY" "$ROOT/scripts/stream_windowed_agg.py" 2>&1 | grep -aoE 'VAJRA_WAGG.*' || true
