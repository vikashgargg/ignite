...
# deleted at the end ($0 discipline). Vajra reaches S3 via the object-store `from_env` (node instance role).
# Usage: scripts/eks_p1_s3_eo.sh [N]
set -uo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"; cd "$ROOT"
N="${1:-100000000}"; REGION=ap-south-1; NS=stream
BUCKET="vajra-p1-$(date +%s)"
...
kk set env deploy/vajra-stream AWS_REGION="$REGION" >/dev/null   # object-store from_env uses node instance role
...
run_wagg() { # $1=out-subdir  -> emits VAJRA_WAGG line
  kk exec vajra-client -- sh -c \
    "SPARK_REMOTE=sc://vajra-stream.$NS.svc.cluster.local:50051 BOOT=kafka.$NS.svc.cluster.local:9092 TOPIC=events N_EVENTS=$N OUT=s3://$BUCKET/$1 CK=s3://$BUCKET/$1_ck python3 /tmp/wagg.py" 2>&1
}
verify_s3() { # $1=out-subdir -> read the S3 parquet, assert groups=9000 + no dup (distinct windows*keys)
  kk exec vajra-client -- sh -c "SPARK_REMOTE=sc://vajra-stream.$NS.svc.cluster.local:50051 python3 - <<PY
from pyspark.sql import SparkSession, functions as F
s=SparkSession.builder.remote('sc://vajra-stream.$NS.svc.cluster.local:50051').getOrCreate()
...
WAGG=$(run_wagg wagg_clean); echo "$WAGG" | grep -aoE 'VAJRA_WAGG.*'
...
kk exec vajra-client -- sh -c \
  "SPARK_REMOTE=sc://vajra-stream.$NS.svc.cluster.local:50051 BOOT=kafka.$NS.svc.cluster.local:9092 TOPIC=events N_EVENTS=$N OUT=s3://$BUCKET/wagg_crash CK=s3://$BUCKET/wagg_crash_ck python3 /tmp/wagg.py" >/tmp/p1b.log 2>&1 &
RUNPID=$!
sleep 8
echo "== CHAOS: kill -9 vajra-stream server =="; kk delete pod -l app=vajra-stream --grace-period=0 --force >/dev/null 2>&1
...
run_wagg wagg_crash | grep -aoE 'VAJRA_WAGG.*' || true
