#!/usr/bin/env python3
"""Prove Vajra's windowed-agg output is IDENTICAL to Flink's — the "same S3/sink files as Flink" check.

Vajra output: parquet under a local dir (aws s3 cp'd), columns [window{start,end}, k, count].
...
Usage: compare_vajra_flink_output.py <vajra_parquet_dir> <flink_jsonl>
"""
import sys, json, glob, datetime
import pyarrow.parquet as pq
import pyarrow as pa

vajra_dir, flink_jsonl = sys.argv[1], sys.argv[2]

# --- Vajra ---
files = [f for f in glob.glob(f"{vajra_dir}/**/*.parquet", recursive=True) if "_spark_metadata" not in f]
...
print(f"VAJRA groups={len(vaj)} sum={sum(vaj.values())} windows={len(set(k[0] for k in vaj))}")
print(f"FLINK groups={len(flk)} sum={sum(flk.values())} windows={len(set(k[0] for k in flk))}")
print(f"only_in_vajra={len(only_v)} only_in_flink={len(only_f)} value_mismatches={len(mismatch)}")
if mismatch:
