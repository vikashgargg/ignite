...
# Kafka -> event-time windowed COUNT -> Parquet on s3://vajra/... (MinIO). Local + FREE; the S3 path
...
BIN="$ROOT/target/release/vajra"; [ -x "$BIN" ] || BIN="$ROOT/target/debug/vajra"
PY="$ROOT/.venvs/smoke/bin/python"
KPOD=$(docker ps --format '{{.Names}}' | grep -i kafka | head -1)
[ -x "$BIN" ] && [ -x "$PY" ] && [ -n "$KPOD" ] || { echo "FATAL: need vajra bin + .venvs/smoke + kafka container"; exit 2; }
echo "=== E2E verify on MinIO S3: bin=$(basename $(dirname $BIN))/vajra N=$N keys=$KEYS ==="
...
S3OUT="s3://vajra/wagg"; S3CK="s3://vajra/ck"
...
mc "mc rm -r --force local/vajra/wagg local/vajra/ck >/dev/null 2>&1; echo bucket-reset"
...
pkill -9 -f 'target/(debug|release)/vajra' 2>/dev/null; sleep 1
# VAJRA_COMPLETE_ON_END=1 = bounded-complete flush (Flink scan.bounded.mode parity) so the FINAL window
# flushes and Σ counts == N EXACT (default = Spark availableNow, drops the last window).
VAJRA_COMPLETE_ON_END=1 VAJRA_WM_PROF=1 RUST_LOG="warn,sail_physical_plan::streaming::window_accum=info" \
...
# Peak-RSS sampler over all vajra processes (memory axis).
( PEAK=0; while kill -0 "$SRV" 2>/dev/null; do
    R=$(ps -o rss= -p $(pgrep -f 'target/(debug|release)/vajra' | tr '\n' ',' | sed 's/,$//') 2>/dev/null | awk '{s+=$1} END{print s}')
...
WAGG=$(SPARK_REMOTE="sc://localhost:$PORT" BOOT="$BOOT" TOPIC="$TOPIC" N_EVENTS="$N" MAXOFFSETS="$N" \
  OUT="$S3OUT" CK="$S3CK" timeout 600 "$PY" scripts/stream_windowed_agg.py 2>&1 | grep -aoE 'VAJRA_WAGG.*' | tail -1)
