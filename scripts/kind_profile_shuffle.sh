...
# (what local loopback can't show). Deploys Kafka+MinIO+distributed vajra with VAJRA_PPROF on the WORKER
...
# Assumes: TAG=pprof2 scripts/kind_up.sh (cluster up + vajra:pprof2 loaded). Usage: N=20000000 TAG=pprof2 scripts/kind_profile_shuffle.sh
set -uo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"; cd "$ROOT"
N="${N:-20000000}"; KEYS="${KEYS:-1000}"; PARTS="${PARTS:-4}"; TAG="${TAG:-pprof2}"; NS=stream; CTX="${CTX:-kind-vajra-kind}"
...
echo "==== [2] deploy distributed vajra with VAJRA_PPROF_SECS=$PPROF_SECS on WORKERS ===="
EXTRA='{"name":"VAJRA_WM_PROF","value":"1"},{"name":"VAJRA_COMPLETE_ON_END","value":"1"},{"name":"VAJRA_PPROF_SECS","value":"'"$PPROF_SECS"'"},{"name":"VAJRA_PPROF_OUT","value":"/tmp/prof.folded"},{"name":"AWS_ENDPOINT","value":"'"$MINIO_EP"'"},{"name":"AWS_ENDPOINT_URL","value":"'"$MINIO_EP"'"},{"name":"AWS_ACCESS_KEY_ID","value":"minioadmin"},{"name":"AWS_SECRET_ACCESS_KEY","value":"minioadmin"},{"name":"AWS_ALLOW_HTTP","value":"true"},{"name":"AWS_REGION","value":"us-east-1"},{"name":"VAJRA_SHUFFLE_BATCH_ROWS","value":"16384"}'
...
kk set env deploy/vajra-driver AWS_REGION=us-east-1 AWS_ENDPOINT="$MINIO_EP" AWS_ENDPOINT_URL="$MINIO_EP" \
   ...
   SAIL_CLUSTER__WORKER_TASK_SLOTS="${SLOTS:-8}" VAJRA_COMPLETE_ON_END=1 VAJRA_SHUFFLE_BATCH_ROWS=16384 >/dev/null
kk wait --for=condition=available --timeout=240s deployment/vajra-driver || { echo "driver failed"; kk get pods -o wide | sed -E 's/[0-9]{12}/<ACCT>/g'; exit 1; }
T0=$(date +%s)
...
for r in $(seq 1 "$RUNS"); do
  R=$(kk exec vajra-client -- sh -c "SPARK_REMOTE=$SR BOOT=$BOOT TOPIC=events N_EVENTS=$N MAXOFFSETS=500000 OUT=s3://vajra/prof$r CK=s3://vajra/prof${r}_ck python3 /tmp/wagg.py" 2>&1 | grep -aoE 'VAJRA_WAGG.*' | tail -1)
  echo "  run$r: $R"
done
