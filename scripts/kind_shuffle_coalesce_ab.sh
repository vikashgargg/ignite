...
N="${N:-10000000}"; KEYS="${KEYS:-1000}"; TAG="${TAG:-shufcoal3}"; NS=stream; CTX="${CTX:-kind-vajra-kind}"
kk() { kubectl --context "$CTX" -n "$NS" "$@"; }
scale_kind() {  # fit the kind Docker VM: shrink cpu/mem requests + worker template cpu 3->1
  sed -E -e 's/cpu: "1[0-9]"/cpu: "1"/g' -e 's/cpu: "[3-9]"/cpu: "1"/g' \
         -e 's/memory: "2[0-9]Gi"/memory: "2Gi"/g' -e 's/memory: "1[0-9]Gi"/memory: "1500Mi"/g' \
         -e 's/"cpu":"3"/"cpu":"1"/g' -e 's/"memory":"10Gi"/"memory":"1500Mi"/g'
}
...
if [ "${CUR:-0}" != "$N" ]; then
  ...
  sed -e "s|N_EVENTS, value: \"[0-9]*\"|N_EVENTS, value: \"$N\"|" k8s/stream/producer-job.yaml | scale_kind | kk apply -f -
...
run() { # $1=rows $2=label
  ...
  local EXTRA='{"name":"VAJRA_WM_PROF","value":"1"},{"name":"AWS_ENDPOINT","value":"'"$MINIO_EP"'"},{"name":"AWS_ENDPOINT_URL","value":"'"$MINIO_EP"'"},{"name":"AWS_ACCESS_KEY_ID","value":"minioadmin"},{"name":"AWS_SECRET_ACCESS_KEY","value":"minioadmin"},{"name":"AWS_ALLOW_HTTP","value":"true"},{"name":"AWS_REGION","value":"us-east-1"},{"name":"VAJRA_SHUFFLE_BATCH_ROWS","value":"'"$ROWS"'"}'
  ...
  local R; R=$(kk exec vajra-client -- sh -c \
    "SPARK_REMOTE=$SR BOOT=$BOOT TOPIC=events N_EVENTS=$N MAXOFFSETS=1000000 OUT=s3://vajra/$LABEL CK=s3://vajra/${LABEL}_ck python3 /tmp/wagg.py" 2>&1 | grep -aoE 'VAJRA_WAGG.*' | tail -1)
  sleep 11
  local SB=0
