...
# the distributed windowed-agg twice: coalescing OFF (ZELOX_SHUFFLE_BATCH_ROWS=0) then ON (16384). Asserts
...
N="${N:-2000000}"; KEYS="${KEYS:-1000}"; PARTS="${PARTS:-2}"; TAG="${TAG:-shufcoal3}"; NS=stream; CTX="${CTX:-kind-zelox-kind}"
...
run() { # $1=rows $2=label
  ...
  # inject MinIO S3 env + shuffle-batch-rows into the worker pod template (after ZELOX_WM_PROF) + driver
  # NOTE: the window runs on WORKERS, so ZELOX_COMPLETE_ON_END (bounded end-flush) + MinIO S3 env MUST be
  # on the worker template, not just the driver — else windows accumulate state but never emit (no output).
  local EXTRA='{"name":"ZELOX_WM_PROF","value":"1"},{"name":"ZELOX_COMPLETE_ON_END","value":"1"},{"name":"AWS_ENDPOINT","value":"'"$MINIO_EP"'"},{"name":"AWS_ENDPOINT_URL","value":"'"$MINIO_EP"'"},{"name":"AWS_ACCESS_KEY_ID","value":"minioadmin"},{"name":"AWS_SECRET_ACCESS_KEY","value":"minioadmin"},{"name":"AWS_ALLOW_HTTP","value":"true"},{"name":"AWS_REGION","value":"us-east-1"},{"name":"ZELOX_SHUFFLE_BATCH_ROWS","value":"'"$ROWS"'"}'
  ...
  kk set env deploy/zelox-driver AWS_REGION=us-east-1 AWS_ENDPOINT="$MINIO_EP" AWS_ENDPOINT_URL="$MINIO_EP" \
     ...
     ZELOX_CLUSTER__WORKER_TASK_SLOTS="${SLOTS:-8}" \
     ZELOX_COMPLETE_ON_END=1 ZELOX_SHUFFLE_BATCH_ROWS="$ROWS" >/dev/null
  if ! kk wait --for=condition=available --timeout=240s deployment/zelox-driver >/dev/null 2>&1; then
  ...
  kk delete -f k8s/stream/zelox-stream-dist.yaml --ignore-not-found >/dev/null 2>&1; sleep 3
}
