...
kk cp scripts/stream_windowed_agg_continuous.py zelox-client:/tmp/wagg_co.py
SR="sc://zelox-stream.$NS.svc.cluster.local:50051"; BOOT="kafka.$NS.svc.cluster.local:9092"

run_mx() { # $1=maxOffsets $2=subdir -> ZELOX_WAGG line + peak RSS
  kk rollout restart deploy/zelox-stream >/dev/null 2>&1; wait_ready zelox-stream >/dev/null 2>&1
  until kk logs zelox-client 2>/dev/null | grep -q CLIENT_READY; do sleep 2; done
  local R; R=$(kk exec zelox-client -- sh -c \
    "SPARK_REMOTE=$SR BOOT=$BOOT TOPIC=events N_EVENTS=$N MAXOFFSETS=$1 OUT=s3://$BUCKET/$2 CK=s3://$BUCKET/${2}_ck python3 /tmp/wagg.py" 2>&1 | grep -aoE 'ZELOX_WAGG.*' | tail -1)
  local VPOD MEM; VPOD=$(kk get pod -l app=zelox-stream --field-selector=status.phase=Running -o jsonpath='{.items[0].metadata.name}')
...
for i in $(seq 1 30); do
  sleep 8
  TOT=$(kk exec zelox-client -- sh -c "SPARK_REMOTE=$SR python3 - <<PY
...
echo "Teardown: run scripts/aws_eks_teardown.sh (or eksctl delete cluster --name zelox-stream-ht --region $REGION --force --wait) — NEVER interrupt."
