...
# Requires docker `zelox_kafka`, target/debug/zelox, .venvs/smoke python. Reuses inc_ckpt_gate.py.
...
[ -x "$BIN" ] || { echo "FATAL: no zelox binary"; exit 2; }
...
start() {
  RUST_LOG=warn env ZELOX_RESCALE=1 ZELOX_INC_CKPT=1 ZELOX_STREAMING_STATE_BUDGET_BYTES="$BUDGET" \
