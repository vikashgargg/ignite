...
# (invoked by box_keytrace_launch.sh). Builds the KEY_TRACE-instrumented image, runs Vajra realtime 10M in
# kind→MinIO with VAJRA_KEY_TRACE=1, and captures the per-stage distinct-k census + the S3 per-key result.
...
docker build -f docker/Dockerfile -t vajra:keytrace . 2>&1 | tail -2
...
kind load docker-image vajra:keytrace --name "$CL"
...
echo "###### produce $N + drain (VAJRA_KEY_TRACE active) ######"
kk exec vajra-client -- sh -c "BOOT=$BOOT TOPIC=events N=$N K=1000 EPMS=100 NP=8 CLOSER_TS=1700000200000 python3 /tmp/scale_producer.py 2>&1 | tail -1" || true
SR="sc://vajra-stream.stream.svc.cluster.local:50051"
kk exec vajra-client -- sh -c "AWS_ENDPOINT=$EP AWS_ENDPOINT_URL=$EP AWS_ACCESS_KEY_ID=minioadmin AWS_SECRET_ACCESS_KEY=minioadmin AWS_ALLOW_HTTP=true AWS_REGION=us-east-1 SPARK_REMOTE=$SR BOOT=$BOOT TOPIC=events N_EVENTS=$N RT_DUR='2 seconds' MAX_SECS=500 OUT=s3://vajra/rt_out CK=s3://vajra/rt_ck python3 /tmp/rt_drain.py 2>&1" | grep -aiE "VAJRA_COMPLETENESS" | tail -1
...
kk exec vajra-client -- sh -c "AWS_ENDPOINT=$EP python3 -c \"
...
fs=[k['Key'] for k in c.list_objects_v2(Bucket='vajra',Prefix='rt_out/').get('Contents',[]) if k['Key'].endswith('.parquet') and '_spark_metadata' not in k['Key']]
allk=set();rows=0
for f in fs:
