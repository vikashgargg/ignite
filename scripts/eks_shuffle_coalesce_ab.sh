...
# TWICE on one 100M backlog — coalescing OFF (VAJRA_SHUFFLE_BATCH_ROWS=0) then ON (16384) — on a clean,
...
run_vajra() { # $1=shuffle_batch_rows $2=label -> prints RESULT line + per-pod shuffle batches
  local ROWS="$1" LABEL="$2"
  # Inject VAJRA_SHUFFLE_BATCH_ROWS into BOTH driver env and the worker pod template (coalescing runs in
  ...
  local R; R=$(kk exec vajra-client -- sh -c \
    "SPARK_REMOTE=$SR BOOT=$BOOT TOPIC=events N_EVENTS=$N MAXOFFSETS=4000000 OUT=s3://$BUCKET/$LABEL CK=s3://$BUCKET/${LABEL}_ck python3 /tmp/wagg.py" 2>&1 | grep -aoE 'VAJRA_WAGG.*' | tail -1)
  ...
  kk wait --for=condition=ready --timeout=120s pod/vajra-client >/dev/null 2>&1
  until kk logs vajra-client 2>/dev/null | grep -q CLIENT_READY; do sleep 3; done
}
...
echo "Teardown: scripts/aws_eks_teardown.sh vajra-stream-ht $REGION (NEVER interrupt)."
