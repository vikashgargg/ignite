...
# at full 100M scale + full resources. Phase 1 = Zelox realtime memory+throughput+jeprof (the priority).
...
if [ -n "$ECR" ] && docker pull "${ECR}:prof" 2>/dev/null; then
  docker tag "${ECR}:prof" zelox:prof; echo "pulled ${ECR##*/}:prof (skip build)"
else
  docker build -f docker/Dockerfile --build-arg CARGO_FEATURES=jemalloc --build-arg RELEASE_STRIP=false \
    -t zelox:prof . 2>&1 | tail -3
  [ -n "$ECR" ] && { docker tag zelox:prof "${ECR}:prof" && docker push "${ECR}:prof" >/dev/null 2>&1 && echo "pushed ${ECR##*/}:prof for reuse"; }
fi
...
# produce and lost the topic last time). CPU fits 8 vCPU (kafka 2 + zelox 4). Kafka mem limit 14Gi so
...
echo "############ PHASE 0d: deploy Zelox (prof, 16Gi, MinIO, MALLOC_CONF=prof) ############"
EP="http://minio.stream.svc.cluster.local:9000"
cat > /tmp/zelox-box.yaml <<YAML
apiVersion: apps/v1
kind: Deployment
...
YAML
kk apply -f /tmp/zelox-box.yaml >/dev/null
...
echo "############ PHASE 1: produce $N + Zelox realtime -> MinIO ############"
...
kk exec zelox-client -- sh -c "BOOT=$BOOT TOPIC=events N=$N K=1000 EPMS=1000 NP=8 CLOSER_TS=1700000200000 python3 /tmp/scale_producer.py 2>&1 | tail -2" || true
...
kk exec zelox-client -- sh -c \
 "AWS_ENDPOINT=$EP AWS_ENDPOINT_URL=$EP AWS_ACCESS_KEY_ID=minioadmin AWS_SECRET_ACCESS_KEY=minioadmin AWS_ALLOW_HTTP=true AWS_REGION=us-east-1 \
  SPARK_REMOTE=$SR BOOT=$BOOT TOPIC=events N_EVENTS=$N RT_DUR='5 seconds' MAX_SECS=600 OUT=s3://zelox/rt_out CK=s3://zelox/rt_ck \
  python3 /tmp/rt_drain.py 2>&1" | grep -aiE "ZELOX_|window|sum|EXACT|throughput|drain" | tail -8
VPOD=$(kk get pod -l app=zelox-stream -o jsonpath='{.items[0].metadata.name}')
echo "--- Zelox peak RSS ---"; kk exec "$VPOD" -- sh -c 'cat /sys/fs/cgroup/memory.peak' | awk '{printf "ZELOX_PEAK_RSS=%.2f GiB\n",$1/1073741824}'
kk exec "$VPOD" -- sh -c 'cat /sys/fs/cgroup/memory.stat | grep "^anon "' | awk '{printf "ZELOX_ANON=%.2f GiB\n",$2/1073741824}'
...
if [ -n "$LH" ]; then
  kk cp "$VPOD:$LH" /tmp/zelox.heap
  kk cp "$VPOD:/usr/local/bin/zelox" /tmp/zelox-bin
  docker run --rm -v /tmp:/w debian:12-slim bash -c \
    'apt-get update -q >/dev/null 2>&1; apt-get install -y -q libjemalloc-dev binutils >/dev/null 2>&1; echo "=== JEPROF TOP (inuse_space) ==="; jeprof --text --inuse_space /w/zelox-bin /w/zelox.heap 2>/dev/null | head -20'
else echo "no jeprof heap (alloc < interval)"; fi

echo "############ PHASE 2: Flink realtime 100M apples-to-apples (same events topic + query) ############"
# Sequential: free RAM by removing Zelox first so Flink TM gets the box to itself (fair — Zelox also had
...
# Scale flink-session for the box: TM 12Gi/8 slots (Zelox used 4 workers; 8 slots is a fair-to-generous
...
echo "See ZELOX_PEAK_RSS / ZELOX_ANON / ZELOX_REALTIME_DRAIN / JEPROF TOP / WM_PROF above."
