#!/usr/bin/env bash
# Thorough prod-grade validation sweep for Zelox distributed batch + stateful streaming.
#
# Runs, against a freshly-started Zelox server, the full functional + reliability suite and prints a
...
# Requires: target/release/zelox built; zelox_kafka container up; .venvs/smoke python.
set -uo pipefail
...
start_server() {
  pkill -9 -f "zelox server" 2>/dev/null; sleep 1
...
echo "=== Zelox full validation: mode=$MODE port=$PORT binary=$BIN ==="
start_server
...
pkill -9 -f "zelox server"; sleep 2; start_server
...
pkill -9 -f "zelox server" 2>/dev/null
