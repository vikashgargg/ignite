...
# zelox server (realtime) + docker Kafka + realtime windowed drain -> local parquet -> per-key verify.
...
echo "=== [2] zelox server (realtime, workers=$WORKERS) ==="
...
SPARK_REMOTE="sc://localhost:$PORT" BOOT="$BOOT" TOPIC="$TOPIC" N_EVENTS="$N" RT_DUR="2 seconds" MAX_SECS=200 \
  OUT="$OUT" CK="$CK" "$PY" scripts/stream_realtime_drain.py 2>&1 | grep -aiE "ZELOX_COMPLETENESS|ZELOX_REALTIME" | tail -2
