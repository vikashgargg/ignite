...
YAML
  K apply -f /tmp/rc.yaml >/dev/null; K rollout status deploy/zelox-stream --timeout=150s >/dev/null; }

checkkeys(){ # $1 = out prefix
  K exec zelox-client -- sh -c "$S3ENV OUTP=$1 python3 - <<'PY'
...
fs=[k['Key'] for k in c.list_objects_v2(Bucket='zelox',Prefix=os.environ['OUTP']).get('Contents',[]) if k['Key'].endswith('.parquet') and '_spark_metadata' not in k['Key']]
allk=set();rows=0
for f in fs:
...
run(){ # $1=workers $2=fuse $3=topic $4=outprefix $5=label
  ...
  K exec zelox-client -- sh -c "BOOT=$BOOT TOPIC=$3 N=$N K=1000 EPMS=100 NP=8 CLOSER_TS=1700000200000 python3 /tmp/scale_producer.py >/dev/null 2>&1" || true
  deploy "$1" "$2" "$5"
  K exec zelox-client -- sh -c "$S3ENV SPARK_REMOTE=sc://zelox-stream.stream.svc.cluster.local:50051 BOOT=$BOOT TOPIC=$3 N_EVENTS=$N RT_DUR='2 seconds' MAX_SECS=180 OUT=s3://zelox/$4 CK=s3://zelox/${4}ck python3 /tmp/rt_drain.py 2>&1 | grep -aiE 'ZELOX_COMPLETENESS' | tail -1" || true
  echo -n "  RESULT $5: "; checkkeys "$4/"
}

# ensure client + scripts present
K get pod zelox-client >/dev/null 2>&1 || K apply -f k8s/stream/zelox-client.yaml >/dev/null
until K logs zelox-client 2>/dev/null | grep -q CLIENT_READY; do sleep 5; done
K cp scripts/scale_producer.py zelox-client:/tmp/scale_producer.py; K cp scripts/stream_realtime_drain.py zelox-client:/tmp/rt_drain.py
