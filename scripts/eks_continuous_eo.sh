...
kk cp scripts/stream_windowed_agg_continuous.py zelox-client:/tmp/wcont.py
...
verify() { # $1=out-subdir -> P1_VERIFY rows/distinct/sum/dup
  kk exec zelox-client -- sh -c "SPARK_REMOTE=$SR python3 - <<PY
...
run_cont conteo_clean 90 | grep -aoE 'ZELOX_WAGG.*' || true
...
RUNPID=$!
sleep 25
echo "== CHAOS: kill -9 zelox-stream server (mid continuous run) =="
...
run_cont conteo_crash 90 | grep -aoE 'ZELOX_WAGG.*' || true
