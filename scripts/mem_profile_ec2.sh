...
# 13 GiB). Runs Kafka + Zelox rt43 (server) + a pyspark-4.2 realtime passthrough in Docker, then reads the
# Zelox container's cgroup memory.stat (ANON = real heap/buffers vs FILE = reclaimable page cache) +
# ZELOX_KAFKA_STATS (prefetch bytes) to ATTRIBUTE the RSS. Terminates the box. Grounds M2-M4 before any code.
...
KEY=/tmp/zelox-prof-key.pem; KN="zelox-prof-key-$$"
...
IID="$(aws ec2 run-instances --region "$REGION" --image-id "$AMI" --instance-type "$ITYPE" --key-name "$KN" \
  ...
  --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=zelox-mem-profiler}]' --query 'Instances[0].InstanceId' --output text)"
...
# Remote profiling script: docker net + Kafka + Zelox rt43 server + backlog + realtime passthrough,
# sampling the Zelox container cgroup memory.stat (anon=heap/buffers vs file=page cache) to ATTRIBUTE RSS.
...
# Zelox server — CORRECT args (from k8s manifest): server --ip 0.0.0.0 --port 50051 --mode local-cluster --workers 4
sudo docker run -d --name zelox --network vnet -e RUST_LOG=warn -e ZELOX_KAFKA_STATS=1 \
...
import time
s=SparkSession.builder.remote("sc://zelox:50051").getOrCreate()
...
for i in \$(seq 1 18); do
  sleep 5
  ANON=\$(sudo docker exec zelox sh -c 'cat /sys/fs/cgroup/memory.stat 2>/dev/null | awk "/^anon /{print \\\$2}"')
  FILE=\$(sudo docker exec zelox sh -c 'cat /sys/fs/cgroup/memory.stat 2>/dev/null | awk "/^file /{print \\\$2}"')
  CUR=\$(sudo docker exec zelox sh -c 'cat /sys/fs/cgroup/memory.current 2>/dev/null')
  echo "  t=\${i}0s anon=\$(( \${ANON:-0}/1048576 ))MiB file=\$(( \${FILE:-0}/1048576 ))MiB rss=\$(( \${CUR:-0}/1048576 ))MiB"
done
echo "=== KAFKA_STATS (prefetch) ==="; sudo docker logs zelox 2>&1 | grep -a KAFKA_STATS | tail -3
echo "REMOTE_DONE"
REMOTE
