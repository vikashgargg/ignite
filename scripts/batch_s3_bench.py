...
Connect remote (Vajra OR Spark local[*]) so the SAME code measures both on the same S3 data.

Emits: BATCH_RESULT engine=.. rows=N write_s=.. read_agg_s=.. total_s=.. sum_v=.. distinct_k=..
Usage: SPARK_REMOTE=sc://.. S3_PATH=s3://bucket/p4 N_ROWS=200000000 ENGINE=vajra|spark python batch_s3_bench.py
"""
import os, time
