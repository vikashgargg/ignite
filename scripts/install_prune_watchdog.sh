#!/usr/bin/env bash
# Install the Zelox disk-pressure watchdog as a macOS launchd agent that runs every 10 minutes
...
# copy of the watchdog to a non-protected location (~/.zelox) and point launchd at that. The
# watchdog still `cd`s into $ZELOX_ROOT to prune the real target.
AGENT_DIR="$HOME/.zelox"
...
<dict>
    ...
    <dict><key>ZELOX_ROOT</key><string>${ROOT}</string></dict>
    <key>StartInterval</key><integer>${INTERVAL}</integer>
    <key>RunAtLoad</key><true/>
    <key>StandardOutPath</key><string>/tmp/zelox_prune.launchd.out</string>
    <key>StandardErrorPath</key><string>/tmp/zelox_prune.launchd.err</string>
...
echo "watchdog installed. Log: /tmp/zelox_prune.log"
