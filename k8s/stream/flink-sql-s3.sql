-- Flink realtime windowed COUNT -> S3 (MinIO) parquet. IDENTICAL logical query to Zelox's
-- stream_realtime_drain.py (10s event-time TUMBLE, GROUP BY window+k). Validated on the box: 10 windows,
-- 10000 rows, sum=100,000,000 on MinIO, byte-identical aggregates to Zelox.
SET 'execution.runtime-mode' = 'streaming';
SET 'execution.checkpointing.interval' = '5s';
SET 'parallelism.default' = '8';
-- UTC so window_start renders IDENTICALLY to Zelox's UTC window (else TO_TIMESTAMP_LTZ uses the box TZ
...
CREATE TABLE flink_out (window_start TIMESTAMP(3), k INT, cnt BIGINT)
  WITH ('connector'='filesystem','path'='s3a://zelox/flink_out','format'='parquet');
